...
def test_xpbd_parent_f_includes_control_joint_f(test, device):
    ...
    for joint_kind, joint_force, expected_parent_f in cases:
        with test.subTest(joint_kind=joint_kind):
            ...
            joint_f[int(model.joint_qd_start.numpy()[joint])] = joint_force
            control.joint_f.assign(joint_f)

            solver.step(state_in, state_out, control, None, 1.0 / 60.0)
            parent_f = state_out.body_parent_f.numpy()[link]

            np.testing.assert_allclose(
                parent_f,
                expected_parent_f,
                atol=1.0e-5,
                err_msg=f"{joint_kind}: body_parent_f should include control.joint_f",
            )


def test_xpbd_parent_f_balances_fixed_body_tendon_load(test, device):
    """A fixed joint should report the reaction balancing a stretched tendon."""
    compliance = 1.0e-3
    rest_length = 0.8
    length = 1.0
    ...
    parent_f = state_in.body_parent_f.numpy()[body]
    ...
    np.testing.assert_allclose(
        ...
        rtol=1.0e-4,
        atol=1.0e-2,
        err_msg="Fixed-joint parent reaction should balance the tendon pull on the body",
    )
...
add_function_test(
    TestSolverXPBD,
    "test_xpbd_parent_f_includes_control_joint_f",
    test_xpbd_parent_f_includes_control_joint_f,
    devices=devices,
    check_output=False,
)
