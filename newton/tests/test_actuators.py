...
class TestActuatorStateAssign(unittest.TestCase):
    ...
    def test_assign_copies_built_in_state(self):
        """Copy every array in the built-in delay and PID controller state."""
        device = wp.get_device()
        builder = newton.ModelBuilder()
        link = builder.add_link()
        joint = builder.add_joint_revolute(parent=-1, child=link, axis=newton.Axis.Z)
        ...
        advanced.controller_state.integral.fill_(6.0)
        ...
        np.testing.assert_array_equal(current.controller_state.integral.numpy(), 6.0)

    def test_assign_delegates_to_custom_controller_state(self):
        """Delegate nested and slotted storage to the custom controller state."""
        ...
        class _CustomState(Controller.State):
            ...
            def assign(self, other: Controller.State) -> None:
    ...
    def test_assign_rejects_undecorated_custom_controller_state(self):
    ...
    def test_assign_copies_torch_state_without_aliasing(self):
        """Copy Torch-backed controller state without aliasing the source tensors."""
        with _torch.inference_mode():
            current = ControllerNeuralLSTM.State(hidden=_torch.zeros((1, 1, 1)), cell=_torch.zeros((1, 1, 1)))
            advanced = ControllerNeuralLSTM.State(hidden=_torch.ones((1, 1, 1)), cell=_torch.ones((1, 1, 1)))
...
class TestControllerStateGraphCapture(unittest.TestCase):
    ...
    def _integral_after_all_steps(self, steps_per_graph: int | None, parity_graphs: bool) -> float:
    ...
    def test_pid_integral_advances_per_replay_boundary_assign(self):
        ...
        for steps_per_graph in (None, 1, 2, 3):
            with self.subTest(steps_per_graph=steps_per_graph):
                got = self._integral_after_all_steps(steps_per_graph, parity_graphs=False)
    ...
    def test_pid_integral_advances_per_replay_parity_graphs(self):
        ...
        for steps_per_graph in (1, 2, 3):
            with self.subTest(steps_per_graph=steps_per_graph):
                got = self._integral_after_all_steps(steps_per_graph, parity_graphs=True)
                self.assertAlmostEqual(got, expected, places=6)
