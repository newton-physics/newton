...
class TestEnvironmentGroupCollision(unittest.TestCase):
    ...
    def test_shape_collision_filtering(self):
        ...
        builder.add_shape_box(
            body=body1,
            xform=wp.transform(wp.vec3(0.8, 0, 0), wp.quat_identity()),
            hx=0.5, hy=0.5, hz=0.5,
            cfg=cfg1
        )
    ...
    def test_particle_shape_collision_filtering(self):
        ...
        builder.add_shape_box(
            body=-1,
            xform=wp.transform(wp.vec3(0, 0.2, 0), wp.quat_identity()),
            hx=0.5, hy=0.1, hz=0.5
        )
    ...
    def test_add_builder_environment_groups(self):
        ...
        main_builder.add_shape_box(
            body=-1,
            xform=wp.transform(wp.vec3(0, -1, 0), wp.quat_identity()),
            hx=10, hy=0.1, hz=10,
            cfg=cfg_ground
        )
    ...
    def test_mixed_collision_and_environment_groups(self):
        ...
        builder.add_shape_sphere(body=body_a, xform=wp.transform(wp.vec3(-1, 0, 0), wp.quat_identity()), radius=0.5, cfg=cfg_a)
        # Shape B: collision group 2 (only collides with group 2)
        cfg_b = ModelBuilder.ShapeConfig(collision_group=2)
        builder.add_shape_sphere(body=body_b, xform=wp.transform(wp.vec3(0, 0, 0), wp.quat_identity()), radius=0.5, cfg=cfg_b)
        # Shape C: collision group -1 (collides with everything)
        cfg_c = ModelBuilder.ShapeConfig(collision_group=-1)
        builder.add_shape_sphere(body=body_c, xform=wp.transform(wp.vec3(1, 0, 0), wp.quat_identity()), radius=0.5, cfg=cfg_c)
        
        # Environment 1
        builder.current_env_group = 1
        # Shape D: collision group 1
        cfg_d = ModelBuilder.ShapeConfig(collision_group=1)
        builder.add_shape_sphere(body=body_d, xform=wp.transform(wp.vec3(-1, 2, 0), wp.quat_identity()), radius=0.5, cfg=cfg_d)
        # Shape E: collision group 2
        cfg_e = ModelBuilder.ShapeConfig(collision_group=2)
        builder.add_shape_sphere(body=body_e, xform=wp.transform(wp.vec3(0, 2, 0), wp.quat_identity()), radius=0.5, cfg=cfg_e)
        
        # Global environment
        builder.current_env_group = -1
        # Shape F: collision group 1
        cfg_f = ModelBuilder.ShapeConfig(collision_group=1)
        builder.add_shape_sphere(body=body_f, xform=wp.transform(wp.vec3(0, 4, 0), wp.quat_identity()), radius=0.5, cfg=cfg_f)
