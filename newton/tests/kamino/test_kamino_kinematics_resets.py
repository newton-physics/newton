...
def sample_base_state_wp(model: ModelKamino, rng: np.random.Generator):
    base_q_np, base_u_np = sample_base_state(model.size.num_worlds, rng, max_pos=0.02)
...
def sample_actuator_state_wp(model: ModelKamino, rng: np.random.Generator):
    actuator_q_np = sample_actuator_coords(model, rng, max_pos=0.02, max_angle=np.radians(3.0))[0]
