...
class TestAnymalReset(unittest.TestCase):
    ...
    def _setup_simulation(self, cone_type):
        ...
        self.simulate() 
        self.anymal = ArticulationView(self.model, "/World/envs/env_0/Robot/base", verbose=False, exclude_joint_types=[newton.JOINT_FREE])
        ...
        gravity = wp.array([wp.vec3(0.0, 0.0, -9.81)] * self.num_envs, dtype=wp.vec3, device=self.device)
        self.solver.mjw_model.opt.gravity = gravity
                ...
                if isinstance(initial_value, np.ndarray) and isinstance(current_value, np.ndarray):
                    if initial_value.dtype == bool and current_value.dtype == bool:
                        if not np.array_equal(initial_value, current_value):
                            ...
                            differences.append(f"{attr_name}: {num_different}/{initial_value.size} boolean values differ ({percent_different:.2f}%)")
                        else:
                            identical_count += 1
                    else:
                        if not np.array_equal(initial_value, current_value):
                            ...
                            percent_different = (num_different / initial_value.size) * 100               
                            if num_different > 0: 
                                differences.append(f"{attr_name}: max_diff={max_diff:.10f}, mean_diff={mean_diff:.10f}, shape={initial_value.shape}, {num_different}/{initial_value.size} different values ({percent_different:.2f}%)")
                            else:
                                identical_count += 1
                        else:
                            identical_count += 1
    ...
    def propagate_reset_state(self):
        zero_gravity = wp.array([wp.vec3(0.0, 0.0, 0.0)] * self.num_envs, dtype=wp.vec3, device=self.device)
        self.solver.mjw_model.opt.gravity = zero_gravity        
        self.simulate_step()
    ...
    def _run_reset_test(self, cone_type):
        """Common test logic for both cone types"""
        # Setup simulation with the specified cone type
