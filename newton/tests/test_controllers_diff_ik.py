...
def test_qd_from_y_matches_formula(test: unittest.TestCase, device):
    ...
    np.testing.assert_allclose(joint_qd.numpy(), expected, atol=1e-4)
...
def test_dls_matches_ridge_regression_for_underactuated_robot(test: unittest.TestCase, device):
    ...
    joint_qd = wp.zeros(n_joints, dtype=wp.float32, device=device)
    wp.launch(
        _qd_from_y_kernel,
        dim=n_joints,
        inputs=[jacobian, y, bandwidth, robot_of_dof, slot_of_dof],
    ...
    np.testing.assert_allclose(joint_qd.numpy(), ridge_expected, atol=1e-3)
...
def test_dls_heterogeneous_dof_counts_independent(test: unittest.TestCase, device):
    ...
    joint_qd = wp.zeros(total_dofs, dtype=wp.float32, device=device)
    wp.launch(
        _qd_from_y_kernel,
        dim=total_dofs,
        inputs=[jacobian, y, bandwidth, robot_of_dof, slot_of_dof],
...
def test_dls_zero_damping_is_pseudo_inverse(test: unittest.TestCase, device):
    ...
    joint_qd = wp.zeros(max_dofs, dtype=wp.float32, device=device)
    wp.launch(
        _qd_from_y_kernel,
        dim=max_dofs,
        inputs=[jacobian, y, bandwidth, robot_of_dof, slot_of_dof],
    ...
    np.testing.assert_allclose(joint_qd.numpy(), expected, atol=1e-3)
...
def test_integrate_position_euler_step(test: unittest.TestCase, device):
    ...
    joint_qd = wp.array([1.0, -2.0, 0.5], dtype=wp.float32, device=device)
    dt = wp.array([0.1], dtype=wp.float32, device=device)
    joint_q_target = wp.zeros(3, dtype=wp.float32, device=device)
    wp.launch(
        _integrate_position_kernel, dim=3, inputs=[joint_q, joint_qd, dt], outputs=[joint_q_target], device=device
    )
