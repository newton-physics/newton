...
from newton._src.solvers.vbd.particle_vbd_kernels import evaluate_self_contact_force_norm, leq_n_ring_vertices
...
def test_self_contact_barrier_c2_at_tau(test, device):
    ...
    The log-barrier region (1e-5 < d < tau) and the outer linear-penalty
    ...
    A factor-of-two error in the log-barrier coefficient k2 would halve
    both quantities on the log side, breaking C1 and C2 at once.
    Regression for GitHub issue #2154.
    """
    collision_radius = 0.02
    k = 1.0e3
    tau = collision_radius * 0.5
    eps = tau * 1e-5  # small enough that the numerical limit is tight
    ...
    # C1: force magnitude must be continuous across tau
    ...
    # C2: Hessian scalar must be continuous across tau
    np.testing.assert_allclose(
        d2E[0],
        d2E[1],
        rtol=1e-3,
...
add_function_test(
    TestCollision, "test_self_contact_barrier_c2_at_tau", test_self_contact_barrier_c2_at_tau, devices=devices
)
