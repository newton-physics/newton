...
from newton._src.utils.cable import (
    _cable_body_transforms,
    _cable_spline_points,
    _rotation_minimizing_cable_quaternions,
)
...
def _cable_spline_points_open_impl(test: unittest.TestCase, device):
    ...
    num_segments = 50
    points = _cable_spline_points(control_points, num_segments=num_segments)
    ...
    points_by_length = _cable_spline_points(control_points, segment_length=target)
...
def _cable_spline_points_closed_impl(test: unittest.TestCase, device):
    ...
    num_segments = 40
    points = _cable_spline_points(control_points, num_segments=num_segments, closed=True)
    test.assertEqual(len(points), num_segments + 1)
    ...
    with test.assertRaises(ValueError):
        _cable_spline_points(control_points)  # neither segment spec
    with test.assertRaises(ValueError):
        _cable_spline_points(control_points, num_segments=10, segment_length=0.1)  # both
    with test.assertRaises(ValueError):
        _cable_spline_points(control_points[:2], num_segments=10, closed=True)  # too few
    with test.assertRaises(ValueError):
        _cable_spline_points([control_points[0]], num_segments=10)  # too few (open)
    ...
    points_dup = _cable_spline_points(duplicated, num_segments=num_segments, closed=True)
...
def _rmf_quaternion_alignment_impl(test: unittest.TestCase, device):
    ...
    points = _cable_spline_points(_helix_control_points(), num_segments=40)
    quats = _rotation_minimizing_cable_quaternions(points)
...
def _rmf_planar_curve_no_flip_impl(test: unittest.TestCase, device):
    ...
    points = _cable_spline_points(control_points, num_segments=60)
    quats = _rotation_minimizing_cable_quaternions(points, normal_hint=wp.vec3(0.0, 0.0, 1.0))
...
def _rmf_closed_loop_holonomy_impl(test: unittest.TestCase, device):
    ...
    num_segments = 48
    points = _cable_spline_points(control_points, num_segments=num_segments, closed=True)
    quats = _rotation_minimizing_cable_quaternions(points, closed=True)
...
def _rmf_twist_distribution_impl(test: unittest.TestCase, device):
    ...
    twist_total = np.pi
    quats = _rotation_minimizing_cable_quaternions(points, twist_total=twist_total)
...
def _cable_spline_shape_straight_rest_impl(test: unittest.TestCase, device):
    ...
    control_points = _helix_control_points(num_points=12, turns=1.0)
    shape = newton.utils.cable_spline_shape(control_points, num_segments=24, straight_rest_shape=True)
    ...
    aliased = newton.utils.cable_spline_shape(control_points, num_segments=24)
    ...
    loop = newton.utils.cable_spline_shape(loop_points, num_segments=32, closed=True, straight_rest_shape=True)
...
def _cable_body_transforms_impl(test: unittest.TestCase, device):
    """Verify _cable_body_transforms places body origins at segment midpoints or start points."""
    points = newton.utils.cable_straight_points(
        start=wp.vec3(0.0, 0.0, 0.0), direction=wp.vec3(1.0, 0.0, 0.0), length=1.0, num_segments=4
    )
    quats = newton.utils.rod_parallel_transport_quaternions(points)

    xforms_com = _cable_body_transforms(points, quats, body_frame_origin="com")
    xforms_start = _cable_body_transforms(points, quats, body_frame_origin="start")
    ...
    with test.assertRaises(ValueError):
        _cable_body_transforms(points, quats[:-1])
...
def _add_cable_spline_straight_rest_relaxes_impl(test: unittest.TestCase, device):
    ...
    shape = newton.utils.cable_spline_shape(control_points, num_segments=num_segments)
    xforms = shape.body_transforms(body_frame_origin="com")
    body_q = state0.body_q.numpy()
