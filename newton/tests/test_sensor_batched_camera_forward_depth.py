...
from newton._src.sensors.batched_camera_renderer.utils import (
    Utils,
...
class TestConvertBatchedRayDepthToForwardDepth(unittest.TestCase):
    ...
    def _utils(self):
        render_context = type("_RenderContext", (), {"device": self.device})()
    ...
    def test_identity_transform_3x3(self):
        ...
        camera_count = 1
        depth_image = wp.full((camera_count, height, width), value=uniform_depth, dtype=wp.float32, device=self.device)
        ...
        wp.launch(
            ...
            inputs=[
                ...
                camera_rays,
                camera_transforms,
                self._utils().create_camera_indices(np.zeros(camera_count, dtype=np.int32)),
                out_depth,
            ],
            device=self.device,
        )
    ...
    def test_center_pixel_unchanged(self):
        ...
        depth_image = wp.full((1, height, width), value=uniform_depth, dtype=wp.float32, device=self.device)
        out_depth = wp.zeros_like(depth_image)
        ...
        wp.launch(
            ...
            inputs=[
                ...
                camera_rays,
                camera_transforms,
                self._utils().create_camera_indices(0),
                out_depth,
            ],
            device=self.device,
        )

        center = float(out_depth.numpy()[0, 1, 1])
        self.assertAlmostEqual(
            center,
            uniform_depth,
            places=5,
    ...
    def test_varying_depth(self):
        ...
        depths_np = np.arange(1.0, 1.0 + width * height, dtype=np.float32).reshape(1, height, width)
    ...
    def test_multiple_views_can_share_one_ray_bundle(self):
        """Batched conversion must use camera_indices to select ray bundles."""
        width, height = 3, 3
        fov_rad = math.radians(60.0)
        camera_rays = self._launch_pinhole_rays(width, height, fov_rad)

        view_count = 2
        depth_image = wp.full((view_count, height, width), value=1.0, dtype=wp.float32, device=self.device)
        camera_transforms = wp.array(
            [
        ...
        camera_indices = self._utils().create_camera_indices([0, 1], 0)

        out_depth = self._utils().convert_ray_depth_to_forward_depth(
            depth_image,
            camera_transforms,
            camera_rays,
        ...
        np.testing.assert_allclose(result[1], result[0], rtol=0.0, atol=1e-6)

    def test_create_camera_indices_defaults_to_matching_ray_bundles(self):
        camera_indices = self._utils().create_camera_indices([2, 4, 6])

        np.testing.assert_array_equal(
            camera_indices.numpy(),
            np.array([[2, 0], [4, 1], [6, 2]], dtype=np.int32),
        )

    def test_create_camera_indices_broadcasts_scalar_inputs(self):
        utils = self._utils()

        np.testing.assert_array_equal(
            utils.create_camera_indices([0, 1, 2], 0).numpy(),
            np.array([[0, 0], [1, 0], [2, 0]], dtype=np.int32),
        )
        np.testing.assert_array_equal(
            utils.create_camera_indices(0, [0, 1, 2]).numpy(),
            np.array([[0, 0], [0, 1], [0, 2]], dtype=np.int32),
        )

    def test_create_camera_indices_rejects_mismatched_lengths(self):
        with self.assertRaisesRegex(ValueError, "same length"):
            self._utils().create_camera_indices([0, 1], [0, 1, 2])
