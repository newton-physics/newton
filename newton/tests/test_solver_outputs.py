...
"""Test eager allocation and ownership of contact-indexed solver outputs."""
...
class ContactSolver(newton.solvers.SolverBase):
    ...
    SUPPORTED_OUTPUT_FLAGS = frozenset(newton.solvers.SolverOutputFlags)
...
class CustomContactOutputs(newton.solvers.SolverOutputs):
...
class CustomContactSolver(ContactSolver):
    ...
    OUTPUTS_TYPE = CustomContactOutputs
    SUPPORTED_OUTPUT_FLAGS = ContactSolver.SUPPORTED_OUTPUT_FLAGS | {CustomContactFlags.PRESSURE}
    CONTACT_OUTPUT_FLAGS = ContactSolver.CONTACT_OUTPUT_FLAGS | {CustomContactFlags.PRESSURE}

    def _allocate_outputs(self, outputs, *, requires_grad):
        super()._allocate_outputs(outputs, requires_grad=requires_grad)
        if CustomContactFlags.PRESSURE in outputs:
            outputs.pressure = wp.zeros(
                self.model.rigid_contact_max, dtype=float, device=self.model.device, requires_grad=requires_grad
            )


class TestSolverOutputs(unittest.TestCase):
    """Verify contact capacities are resolved before allocating outputs."""
    ...
    def setUp(self):
        ...
        self.flags = {newton.solvers.SolverOutputFlags.CONTACT_F}
    ...
    def test_uninitialized_capacities(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "CollisionPipeline"):
            self.solver.outputs(self.flags)

    def test_body_only_outputs_need_no_pipeline(self):
        """Allocate body outputs without constructing a collision pipeline."""
        outputs = self.solver.outputs({newton.solvers.SolverOutputFlags.BODY_QDD})
        self.assertEqual(outputs.body_qdd.shape, (self.model.body_count,))
        self.assertIsNone(outputs.contact_f)
    ...
    def test_eager_capacity_allocation(self):
        ...
        outputs = self.solver.outputs(self.flags, requires_grad=True)
        self.assertEqual((self.model.rigid_contact_max, self.model.soft_contact_max), (5, 3))
        self.assertEqual(outputs.contact_f.shape, (8,))
        self.assertTrue(outputs.contact_f.requires_grad)
        self.assertIsNone(outputs.contacts)
        contacts = pipeline.contacts()
        pointer = outputs.contact_f.ptr
        self.solver._validate_outputs(outputs, contacts)
    ...
    def test_zero_capacity_is_requested(self):
        ...
        outputs = self.solver.outputs(self.flags)
        self.assertIsNotNone(outputs.contact_f)
        self.assertEqual(outputs.contact_f.shape, (0,))
        self.assertIsNone(self.solver.outputs(set()).contact_f)
    ...
    def test_validate_contact_layout_and_identity(self):
        ...
        with self.assertRaisesRegex(ValueError, "Contacts"):
            self.solver._validate_outputs(outputs)
        with self.assertRaisesRegex(ValueError, "capacit"):
            self.solver._validate_outputs(outputs, newton.Contacts(4, 4, device="cpu"))
        self.assertIsNone(outputs.contacts)
        contacts = pipeline.contacts()
        self.solver._validate_outputs(outputs, contacts)
        self.solver._validate_outputs(outputs, contacts)
        with self.assertRaisesRegex(ValueError, "Contacts instance"):
            self.solver._validate_outputs(outputs, pipeline.contacts())

    def test_freeze_capacity_after_output_allocation(self):
        """Reject capacity changes that would invalidate allocated outputs."""
    ...
    def test_late_pipeline_failure_preserves_published_capacity(self):
        ...
        self.assertEqual((self.model.rigid_contact_max, self.model.soft_contact_max), (5, 3))
        self.assertEqual(self.solver.outputs(self.flags).contact_f.shape, (8,))

    def test_matching_pipeline_preserves_frozen_capacity(self):
        """Allow another pipeline only if its capacities preserve live outputs."""
        newton.CollisionPipeline(self.model, rigid_contact_max=5, soft_contact_max=3)
        outputs = self.solver.outputs(self.flags)
        matching = newton.CollisionPipeline(self.model, rigid_contact_max=5, soft_contact_max=3)
        self.solver._validate_outputs(outputs, matching.contacts())
        self.assertEqual(outputs.contact_f.shape, (8,))
    ...
    def test_contact_count_does_not_resize_output(self):
        ...
        outputs = ContactSolver(model).outputs(self.flags)
        pointer = outputs.contact_f.ptr
        contacts = pipeline.contacts()
        state = model.state()
        ...
        self.assertEqual(outputs.contact_f.shape, (pipeline.soft_contact_max,))
        self.assertEqual(outputs.contact_f.ptr, pointer)
    ...
    def test_custom_contact_output_without_contact_force(self):
        ...
        self.assertEqual(outputs.pressure.shape, (4,))
        self.assertIsNone(outputs.contact_f)
        solver._validate_outputs(outputs, pipeline.contacts())
        with self.assertRaisesRegex(ValueError, "capacit"):
            self.model.rigid_contact_max = 5
    ...
    def test_native_backend_rejects_insufficient_capacity(self):
        ...
        for solver in (mujoco, kamino):
            self.assertEqual(solver.outputs(self.flags).contact_f.shape, (3,))
    ...
    def test_graph_reuses_preallocated_output(self):
        ...
        outputs = solver.outputs(self.flags)
        contacts = pipeline.contacts()
        pointer = outputs.contact_f.ptr
        with wp.ScopedCapture(device=model.device) as capture:
            solver._validate_outputs(outputs, contacts)
            outputs.contact_f.zero_()
        for _ in range(2):
            wp.capture_launch(capture.graph)
        self.assertEqual(outputs.contact_f.ptr, pointer)
        self.assertEqual(outputs.contact_f.numpy().sum(), 0.0)

    def test_conditional_graph_uses_preallocated_output(self):
        """Use contact outputs in a conditional graph without allocating there."""
        ...
        for _ in range(2):
            outputs.contact_f.fill_(wp.spatial_vector(1.0))
            wp.capture_launch(capture.graph)
            self.assertEqual(outputs.contact_f.numpy().sum(), 0.0)
        self.assertEqual(outputs.contact_f.ptr, pointer)
