...
DEFAULT_TOLERANCES: dict[str, dict[str, float]] = {
    "q": {"rtol": 1e-5, "atol": 1e-4},
    "qd": {"rtol": 1e-5, "atol": 1e-4},
}
...
CASES: list[dict] = [
    # ────────── Cart-pole ──────────
    {
        "example": "cartpole",
        "solver": "euler",
        "policy": "sin-joint-target",
        "num_frames": 10,
        "num_envs": 8,
        "tolerances": {
            "cpu": {
                "body_q": {"rtol": 1e-4, "atol": 1e-3},
                "body_qd": {"rtol": 1e-4, "atol": 5e-2},
            }
        },
    },
    {
    ...
    },
    {
        ...
        "num_frames": 10,
        "num_envs": 8,
        "state_arrays": ("body_q",),  # qd is finite-diffed
    },
    ...
    },
    {
        ...
        "num_envs": 8,
        "state_arrays": ("body_q",),  # qd is finite-diffed
        "solver_kwargs": {"iterations": 64},  # high iterations for convergence (or too non-deterministic for testing)
    },
    {
        "example": "quadruped",
        "solver": "mujoco",
    ...
    # ────────── Cloth self-contact ──────────
    {
        ...
        "num_frames": 10,
        "num_envs": 1,
        "state_arrays": ("particle_q",),  # qd is finite-diffed
        "tolerances": {
            "cpu": {
                "particle_q": {"rtol": 1e-4, "atol": 1e-3},
            }
        },
    },
]
...
def golden_path(
    ...
    return (
        ...
        / "assets"
        / "golden_states"
        / f"{example_name}_{solver_name}_{policy}policy_{num_envs}envs_{num_frames}steps.npz"
    )
        ...
        for arr in state_arrays:
            ...
            try:
                ...
                error_message = (
                    ...
                    f"  Array: {arr}\n"
                    f"  Device: {device}\n\n"
                    f"Please investigate by interactively running and visually inspecting the failed case.\n"
                    f"This failure could be due to one of the following:\n"
                    f"1. A bug has been introduced: Please fix the bug to get the test passing.\n"
                    f"2. Minor numerical drift: Try regenerating the golden states by running:\n"
                    f"   python newton/tests/goldenstate_utils.py\n"
                    f"   You might also need to adjust the test sensitivity (tolerances).\n"
                    f"3. The simulation behavior has intentionally changed and the new behavior is correct: \n"
                    f"   Regenerate the golden states by running:\n"
                    f"   python newton/tests/goldenstate_utils.py\n\n"
                    f"Original error: {e}"
                )
...
def _run_and_compare_mujoco(
    ...
    # Compare the selected state arrays using tolerance resolution logic.
    for arr in state_arrays:
        tol = resolve_tolerance(arr, device=device_mujoco, tolerances=tolerances)
        try:
            np.testing.assert_allclose(state_mujoco[arr], state_native[arr], rtol=tol["rtol"], atol=tol["atol"])
        except AssertionError as e:
            error_message = (
                f"\n\nMuJoCo vs MuJoCo-native comparison failed for:\n"
                f"  Example: {example_name}\n"
                f"  Policy: {policy}\n"
                f"  Num Envs: {num_envs}\n"
                f"  Num Frames: {num_frames}\n"
                f"  Array: {arr}\n"
                f"  MuJoCo (Warp) Device: {device_mujoco}\n"
                f"  MuJoCo-native Device: {device_native}\n\n"
                f"Please investigate by interactively running and visually inspecting the failed case.\n"
                f"This failure could be due to one of the following:\n"
                f"1. A bug has been introduced in either the Warp MuJoCo solver or the example setup.\n"
                f"2. The behavior of MuJoCo or MuJoCo-native has changed, and this difference is significant.\n\n"
                f"Original error: {e}"
            )
            raise AssertionError(error_message) from e
...
for case in CASES:
    ...
    # Create test name
    test_name = f"test_{example}_{solver}_{policy}"
