...
def export_and_reduce_predictive_contact(
    ...
    # Predictive slots encode real zero-based buffer IDs as ID + 1 so packed
    # ID zero can be claimed atomically before allocating buffer storage.
    contact_id = existing_contact_id
    if contact_id >= 0:
        encoded_contact_id = contact_id + 1
        clearance_value = make_contact_value(
            clearance_score, fingerprint, encoded_contact_id, reducer_data.deterministic
        )
        impact_time_value = make_contact_value(
            impact_time_score, fingerprint, encoded_contact_id, reducer_data.deterministic
        )
...
def create_export_reduced_contacts_kernel(writer_func: Any):
    ...
    def export_reduced_contacts_kernel(
        ...
        for i in range(tid, num_active, total_num_threads):
            ...
            entry_key = ht_keys[entry_idx]
            entry_bin = (entry_key >> wp.uint64(55)) & BIN_MASK
            is_predictive_entry = entry_bin == wp.uint64(wp.static(PREDICTIVE_BIN_ID))
            ...
            for slot in range(wp.static(VALUES_PER_KEY)):
                ...
                # Only predictive entries use one-based packed IDs; all
                # underlying contact buffers and regular entries remain zero-based.
                contact_id = unpack_contact_id(value, deterministic)
                if is_predictive_entry:
