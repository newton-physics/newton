...
from .hydroelastic_pressure_fields import (
    PRESSURE_PROFILE_POISSON,
    PressureFieldData,
    build_immutable_pressure_field_from_sdf,
)
...
class HydroelasticRuntimeData:
    ...
    shape_viscoelastic_enabled: np.ndarray
    """Per-shape viscoelastic memory enable flag, shape (shape_count,), dtype int32."""
    shape_viscoelastic_strain_rate_sensitivity: np.ndarray
    """Per-shape strain-rate sensitivity, shape (shape_count,), dtype float32."""
    shape_viscoelastic_densification_exponent: np.ndarray
    """Per-shape densification power-law exponent, shape (shape_count,), dtype float32."""
    shape_viscoelastic_hysteresis_fraction: np.ndarray
    """Per-shape hysteresis energy fraction, shape (shape_count,), dtype float32."""
    shape_viscoelastic_recovery_tau_s: np.ndarray
    """Per-shape memory recovery time constant [s], shape (shape_count,), dtype float32."""
...
def _read_shape_pressure_profiles(model: Model, shape_count: int) -> tuple[np.ndarray, np.ndarray]:
    """Read per-shape pressure profile settings with safe fallback arrays."""
    if hasattr(model, "shape_hydro_pressure_profile") and model.shape_hydro_pressure_profile is not None:
        profile = model.shape_hydro_pressure_profile.numpy().astype(np.int32)
    else:
        profile = np.full(shape_count, PRESSURE_PROFILE_POISSON, dtype=np.int32)

    if hasattr(model, "shape_hydro_pressure_layer_params") and model.shape_hydro_pressure_layer_params is not None:
        layer_params = model.shape_hydro_pressure_layer_params.numpy().astype(np.float32)
    else:
        layer_params = np.tile(np.asarray((0.65, 0.0, 0.0, 0.0), dtype=np.float32), (shape_count, 1))

    return profile, layer_params
...
def _build_pressure_tables(
    model: Model,
    shape_hydro_mode: np.ndarray,
    shape_pressure_profile: np.ndarray,
    shape_pressure_layer_params: np.ndarray,
    ...
    for shape_idx in hydroelastic_indices:
        ...
        profile = int(shape_pressure_profile[shape_idx])
        layer_params = np.asarray(shape_pressure_layer_params[shape_idx], dtype=np.float32)
        workflow = int(shape_contact_workflow[shape_idx])

        if workflow != int(HydroelasticContactWorkflow.PRESSURE):
            profile = PRESSURE_PROFILE_POISSON
            layer_params = np.asarray((0.65, 0.0, 0.0, 0.0), dtype=np.float32)
        ...
        key = (
            int(sdf_idx),
            int(profile),
            float(layer_params[0]),
            float(layer_params[1]),
            float(layer_params[2]),
            float(layer_params[3]),
            float(amp[0]),
            float(amp[1]),
            float(amp[2]),
            float(cyc[0]),
        ...
        if pressure_idx is None:
            ...
            pressure_data, pressure_volume = build_immutable_pressure_field_from_sdf(
                ...
                pressure_layer_params=layer_params,
            )
...
def _read_shape_viscoelastic_fields(model: Model, shape_count: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Read per-shape viscoelastic config arrays with safe fallback defaults.

    The fallback defaults match :class:`HydroelasticSDF.Config` defaults.
    """
    if hasattr(model, "shape_viscoelastic_enabled") and model.shape_viscoelastic_enabled is not None:
        enabled = model.shape_viscoelastic_enabled.numpy().astype(np.int32)
    else:
        enabled = np.zeros(shape_count, dtype=np.int32)

    if hasattr(model, "shape_viscoelastic_strain_rate_sensitivity") and model.shape_viscoelastic_strain_rate_sensitivity is not None:
        strain_rate_sensitivity = model.shape_viscoelastic_strain_rate_sensitivity.numpy().astype(np.float32)
    else:
        strain_rate_sensitivity = np.zeros(shape_count, dtype=np.float32)

    if hasattr(model, "shape_viscoelastic_densification_exponent") and model.shape_viscoelastic_densification_exponent is not None:
        densification_exponent = model.shape_viscoelastic_densification_exponent.numpy().astype(np.float32)
    else:
        densification_exponent = np.full(shape_count, 2.0, dtype=np.float32)

    if hasattr(model, "shape_viscoelastic_hysteresis_fraction") and model.shape_viscoelastic_hysteresis_fraction is not None:
        hysteresis_fraction = model.shape_viscoelastic_hysteresis_fraction.numpy().astype(np.float32)
    else:
        hysteresis_fraction = np.full(shape_count, 0.3, dtype=np.float32)

    if hasattr(model, "shape_viscoelastic_recovery_tau_s") and model.shape_viscoelastic_recovery_tau_s is not None:
        recovery_tau_s = model.shape_viscoelastic_recovery_tau_s.numpy().astype(np.float32)
    else:
        recovery_tau_s = np.full(shape_count, 0.25, dtype=np.float32)

    return enabled, strain_rate_sensitivity, densification_exponent, hysteresis_fraction, recovery_tau_s
...
def collect_hydroelastic_runtime_data(model: Model) -> HydroelasticRuntimeData | None:
    ...
    shape_pressure_profile, shape_pressure_layer_params = _read_shape_pressure_profiles(model, model.shape_count)
    ...
    shape_pressure_index, compact_pressure_field_data, pressure_field_volume = _build_pressure_tables(
        model,
        shape_hydro_mode,
        shape_pressure_profile,
        shape_pressure_layer_params,
    ...
    (
        shape_viscoelastic_enabled,
        shape_viscoelastic_strain_rate_sensitivity,
        shape_viscoelastic_densification_exponent,
        shape_viscoelastic_hysteresis_fraction,
        shape_viscoelastic_recovery_tau_s,
    ) = _read_shape_viscoelastic_fields(model, model.shape_count)
    ...
    return HydroelasticRuntimeData(
        ...
        shape_viscoelastic_enabled=shape_viscoelastic_enabled,
        shape_viscoelastic_strain_rate_sensitivity=shape_viscoelastic_strain_rate_sensitivity,
        shape_viscoelastic_densification_exponent=shape_viscoelastic_densification_exponent,
        shape_viscoelastic_hysteresis_fraction=shape_viscoelastic_hysteresis_fraction,
        shape_viscoelastic_recovery_tau_s=shape_viscoelastic_recovery_tau_s,
    )
