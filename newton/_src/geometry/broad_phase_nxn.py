...
def _nxn_broadphase_precomputed_pairs(
    # Input arrays
    shape_bounding_box_lower: wp.array1d[wp.vec3],
    shape_bounding_box_upper: wp.array1d[wp.vec3],
    shape_gap: wp.array1d[float],  # Optional per-shape effective gaps (can be empty if AABBs pre-expanded)
    nxn_shape_pair: wp.array1d[wp.vec2i],
    # Output arrays
...
def _find_world_and_local_id(
    tid: int,
    world_cumsum_lower_tri: wp.array1d[int],
):
...
def _nxn_broadphase_kernel(
    ...
    shape_bounding_box_upper: wp.array1d[wp.vec3],
    shape_gap: wp.array1d[float],  # Optional per-shape effective gaps (can be empty if AABBs pre-expanded)
    collision_group: wp.array1d[int],  # per-shape
    shape_world: wp.array1d[int],  # per-shape world indices
    world_cumsum_lower_tri: wp.array1d[int],  # Cumulative sum of lower tri elements per world
    world_slice_ends: wp.array1d[int],  # End indices of each world slice
    world_index_map: wp.array1d[int],  # Index map into source geometry
    num_regular_worlds: int,  # Number of regular world segments (excluding dedicated -1 segment)
    filter_pairs: wp.array1d[wp.vec2i],  # Sorted excluded pairs (empty if none)
...
class BroadPhaseAllPairs:
    ...
    def __init__(
        self,
        shape_world: wp.array1d[wp.int32] | np.ndarray,
        shape_flags: wp.array1d[wp.int32] | np.ndarray | None = None,
    ...
    def launch(
        self,
        shape_lower: wp.array1d[wp.vec3],  # Lower bounds of shape bounding boxes
        shape_upper: wp.array1d[wp.vec3],  # Upper bounds of shape bounding boxes
        shape_gap: wp.array1d[float] | None,  # Optional per-shape effective gaps
        shape_collision_group: wp.array1d[int],  # Collision group ID per box
        shape_world: wp.array1d[int],  # World index per box
        shape_count: int,  # Number of active bounding boxes
        # Outputs
        candidate_pair: wp.array1d[wp.vec2i],  # Array to store overlapping shape pairs
        candidate_pair_count: wp.array1d[int],
        device: Devicelike | None = None,  # Device to launch on
        filter_pairs: wp.array1d[wp.vec2i] | None = None,  # Sorted excluded pairs
...
class BroadPhaseExplicit:
    ...
    def launch(
        ...
        shape_upper: wp.array1d[wp.vec3],  # Upper bounds of shape bounding boxes
        shape_gap: wp.array1d[float] | None,  # Optional per-shape effective gaps
        shape_pairs: wp.array1d[wp.vec2i],  # Precomputed pairs to check
