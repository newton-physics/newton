...
@wp.func
def texture_sample_sdf_value_only(
    sdf: TextureSDFData,
    local_pos: wp.vec3,
) -> float:
    """Sample only the SDF value using the gradient sampler's arithmetic order.

    Unlike :func:`texture_sample_sdf`, this helper de-quantizes sparse-subgrid
    corners before interpolation. This matches the value returned by
    :func:`texture_sample_sdf_grad` while avoiding its unused gradient work.
...
def texture_sample_sdf_grad_only(
    ...
    """Sample only the gradient using the software trilinear sampler.

    The corner reads and gradient arithmetic match
    :func:`texture_sample_sdf_grad`; only the unused value interpolation is
    omitted.
