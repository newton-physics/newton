...
def test_type_affinity_pair(
    ...
    Two entities collide if either entity accepts the other's type bits.

    Args:
        collision_type_a: Collision type bits for entity A.
        collision_affinity_a: Collision affinity bits accepted by entity A.
        collision_type_b: Collision type bits for entity B.
        collision_affinity_b: Collision affinity bits accepted by entity B.

    Returns:
        True when either entity accepts the other's type bits.
    """
...
def test_world_and_type_affinity_pair(
    ...
    Args:
        world_a: World index for entity A, or -1 for a global entity.
        world_b: World index for entity B, or -1 for a global entity.
        collision_type_a: Collision type bits for entity A.
        collision_affinity_a: Collision affinity bits accepted by entity A.
        collision_type_b: Collision type bits for entity B.
        collision_affinity_b: Collision affinity bits accepted by entity B.
    ...
    Returns:
        True if the entities share a compatible world and type/affinity masks.
    """
    if world_a != -1 and world_b != -1 and world_a != world_b:
        return False
...
def compile_group_masks_for_broad_phase(
    ...
    Positive groups collide with themselves and all negative groups. Negative
    groups collide with all groups except the same negative group. Group 0
    produces zero masks.

    Args:
        collision_group: Per-shape collision group IDs.
        device: Device for the output Warp arrays. If None, uses the input
            Warp array device or CPU for host inputs.

    Returns:
        Tuple of collision type and collision affinity Warp arrays.

    Raises:
        ValueError: If more than 32 non-zero collision groups are provided.
    """
