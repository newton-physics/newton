...
def create_soft_contacts(
    soft_contact_pairs: wp.array[wp.vec2i],
...
def compute_tri_bvh_bounds(
    ...
    v1_index = tri_indices[t_id, 0]
    v2_index = tri_indices[t_id, 1]
    v3_index = tri_indices[t_id, 2]
    ...
    world_index = particle_world[v1_index]
    if world_index < 0:
...
def compute_edge_bvh_bounds(
    ...
    v1_index = edge_indices[e_id, 2]
    v2_index = edge_indices[e_id, 3]
...
def vertex_triangle_collision_detection_kernel(
    ...
    # Fixed two-pass query: local vertices query their world group, then the group
    # holding world -1 primitives. World -1 vertices query the whole BVH once.
...
def edge_colliding_edges_detection_kernel(
    ...
    # Fixed two-pass query: local edges query their world group, then the group
    # holding world -1 primitives. World -1 edges query the whole BVH once.
    for query_pass in range(2):
        run_query = bool(False)
        query_all = bool(False)
        group_root = wp.int32(-1)
