...
def parse_usd(
    ...
    *,
    ...
    def _builder_body_xform(body_id: int) -> wp.transform:
    ...
    def _import_cable_graphs() -> tuple[set[str], set[str]]:
        ...
        for prim in Usd.PrimRange(root_prim, Usd.TraverseInstanceProxies()):
            ...
            if not pts or not vcounts or len(vcounts) != 1 or int(vcounts[0]) < 3:
                continue
            wmat = _get_prim_world_mat(prim, None, incoming_world_xform)
        ...
        for prim in Usd.PrimRange(root_prim, Usd.TraverseInstanceProxies()):
            ...
            if src0 not in curve_recs or src1 not in curve_recs or src0 == src1:
                continue
            if (
                str(deformable_read(prim, "type0") or "") != "point"
                or str(deformable_read(prim, "type1") or "") != "point"
            ):
                continue
    ...
    consumed_cable_curve_paths, consumed_junction_attachment_paths = _import_cable_graphs()
    ...
    def _attachment_vec3_tuples(values: list[wp.vec3]) -> list[tuple[float, float, float]]:
    ...
    def _attachment_world_point_from_xform(target_path: str, local_point: wp.vec3) -> tuple[int, wp.vec3] | None:
        ...
        target_mat = _get_prim_world_mat(target_prim, None, incoming_world_xform)
        ...
        if body_idx < 0:
            return None
        local_body_point = wp.transform_point(wp.transform_inverse(_builder_body_xform(body_idx)), world_point)
    ...
    def _cable_attachment_anchors(
        ...
        segment_map = path_cable_segments.get(src_path)
        point_anchors = path_cable_point_anchors.get(src_path)
    ...
    def _mark_attachment_unsupported(attrs: dict[str, Any], path: str, reason: str) -> None:
    ...
    if root_prim and root_prim.IsValid():
        for prim in Usd.PrimRange(root_prim, Usd.TraverseInstanceProxies()):
            ...
            src0 = _get_first_target(prim, "physics:src0")
            src1 = _get_first_target(prim, "physics:src1")
            ...
            if not indices0:
                _mark_attachment_unsupported(
                    attrs, path, "PhysicsAttachment has no indices0 attachment sites; skipping."
                )
                continue
            ...
            for site_idx, src_index in enumerate(indices0):
                ...
                cable_anchors = _cable_attachment_anchors(path, src0, type0, src_index, coord0)
