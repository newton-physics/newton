...
# Cable spline sampling and rotation-minimizing frames
...
def _cable_spline_points(
    ...
    *,
    ...
    if (num_segments is None) == (segment_length is None):
        raise ValueError("cable_spline_shape: provide exactly one of num_segments and segment_length")
    if num_segments is not None and num_segments < 2:
        raise ValueError("cable_spline_shape: num_segments must be >= 2")
    if segment_length is not None and (not math.isfinite(segment_length) or segment_length <= 0.0):
        raise ValueError("cable_spline_shape: segment_length must be positive and finite")
    if not 0.0 <= alpha <= 1.0:
        raise ValueError("cable_spline_shape: alpha must be in [0, 1]")
    if oversampling < 2:
        raise ValueError("cable_spline_shape: oversampling must be >= 2")

    points = np.array([[float(p[0]), float(p[1]), float(p[2])] for p in control_points], dtype=np.float64)
    if not np.isfinite(points).all():
        raise ValueError("cable_spline_shape: control_points must be finite")
    points = _merge_duplicate_control_points(points, closed)

    min_points = 3 if closed else 2
    if points.shape[0] < min_points:
        raise ValueError(
            f"cable_spline_shape: need at least {min_points} distinct control points for a "
    ...
    if total_length <= 0.0:
        raise ValueError("cable_spline_shape: curve has zero length")
...
def _rotation_minimizing_cable_quaternions(
    points: Sequence[wp.vec3],
    *,
    twist_total: float = 0.0,
    ...
    Like :func:`rod_parallel_transport_quaternions`, the returned quaternions rotate the
...
class CableSplineShape(NamedTuple):
    """Initial and rest geometry of a spline cable, as produced by :func:`cable_spline_shape`.
    ...
    *rest* (zero-strain) configuration. When the rest shape is the spline itself, the rest
    ...
    def body_transforms(
        ...
        *,
        ...
        """Return per-body transforms for writing this shape into :attr:`State.body_q`.

        Args:
            rest: If True, use the rest configuration (``rest_points``/``rest_quaternions``);
                otherwise the posed spline configuration (``points``/``quaternions``).
            body_frame_origin: Body-frame placement matching the rod build: ``"com"`` places the
                body origin at the segment midpoint, ``"start"`` at the segment's first point.

        Returns:
            A list of ``wp.transform``, one per segment body.
        """
        points = self.rest_points if rest else self.points
        quaternions = self.rest_quaternions if rest else self.quaternions
        return _cable_body_transforms(points, quaternions, body_frame_origin=body_frame_origin)
...
def cable_spline_shape(
    ...
    *,
    ...
    rotation-minimizing frames (double reflection method). By default the rest geometry is the
    ...
    into the simulation state — see ``straight_rest_shape`` in
    :meth:`ModelBuilder.add_cable_spline` and :meth:`CableSplineShape.body_transforms`.
    ...
    quaternions = _rotation_minimizing_cable_quaternions(
        points,
        twist_total=twist_total,
    ...
    if closed:
        ...
        rest_quaternions = _rotation_minimizing_cable_quaternions(rest_points, normal_hint=normal_hint, closed=True)
    else:
        ...
        rest_points = [wp.vec3(*p) for p in rest_pts]
        rest_quaternions = _rotation_minimizing_cable_quaternions(rest_points, normal_hint=normal_hint)
    ...
    if max_deviation > 0.9 * math.pi:
        warnings.warn(
            f"cable_spline_shape: maximum per-joint rotation between the rest and posed "
...
def _cable_body_transforms(
    ...
    *,
    ...
    if len(quaternions) != num_segments:
        raise ValueError(
            f"CableSplineShape.body_transforms: expected {num_segments} quaternions for "
            f"{num_segments} segments, got {len(quaternions)}"
        )
    if body_frame_origin not in ("start", "com"):
        raise ValueError("CableSplineShape.body_transforms: body_frame_origin must be 'start' or 'com'")
