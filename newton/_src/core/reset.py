...
def validate_reset_world_mask(
    ...
    *,
    ...
    ``allow_legacy`` additionally permits the deprecated ``world_count`` shape
    so established public solver APIs can normalize it at their boundary.
    """
    ...
    if world_mask.device != device:
        raise ValueError(f"'world_mask' device {world_mask.device} does not match solver device {device}.")
    mask_size = world_mask.shape[0]
    if mask_size == world_count + 1 or (allow_legacy and mask_size == world_count):
        return world_mask
    if allow_legacy:
        raise ValueError(
            f"world_mask has size {mask_size}, expected {world_count} or {world_count + 1}."
        )
    raise ValueError(
        f"'world_mask' length {mask_size} must equal model.world_count + 1 ({world_count + 1})."
    )
