...
def create_kernel(
    ...
    def render_megakernel(
        ...
        out_index = world_index * pixels_per_view + py * img_width + px

        world_render_flag = world_render_flags[world_index]
        if (world_render_flag & wp.static(int(WorldRenderFlag.ENABLE))) == 0:
            if (world_render_flag & wp.static(int(WorldRenderFlag.DISABLE_CLEAR))) != 0:
                write_clear_outputs(
                    out_index,
                    out_color,
                    out_depth,
