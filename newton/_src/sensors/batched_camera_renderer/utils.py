...
def flatten_color_image(
    ...
    width: wp.int32,
    height: wp.int32,
    cameras_per_row: wp.int32,
):
    camera_id, y, x = wp.tid()

    row = camera_id // cameras_per_row
    col = camera_id % cameras_per_row

    px = col * width + x
    py = row * height + y
    color = color_image[camera_id, y, x]
...
def flatten_normal_image(
    ...
    px = col * width + x
    py = row * height + y
    normal = normal_image[camera_id, y, x] * 0.5 + wp.vec3f(0.5)
...
def find_depth_range(depth_image: wp.array3d[wp.float32], depth_range: wp.array[wp.float32]):
    camera_id, y, x = wp.tid()
    depth = depth_image[camera_id, y, x]
    if depth > 0:
...
def convert_ray_depth_to_forward_depth_kernel(
    ...
    view_index, py, px = wp.tid()

    ray_bundle_index = camera_indices[view_index, 1]
    if camera_indices[view_index, 0] < 0 or ray_bundle_index < 0:
        out_depth[view_index, py, px] = 0.0
        return

    ray_depth = depth_image[view_index, py, px]
    camera_transform = camera_transforms[view_index]
    camera_ray = camera_rays[ray_bundle_index, py, px, 1]
    ...
    out_depth[view_index, py, px] = ray_depth * wp.dot(ray_dir_world, cam_forward_world)
...
def unpack_normal_to_rgba_kernel(
    ...
    """Unpack ``(camera_count, height, width)`` vec3 normals into ``(camera_count, height, width, 4)`` RGBA.
    ...
    out[camera_index, py, px, 0] = r
    out[camera_index, py, px, 1] = g
    out[camera_index, py, px, 2] = b
    out[camera_index, py, px, 3] = wp.uint8(255)
...
def unpack_depth_to_rgba_kernel(
    ...
    """Unpack ``(camera_count, height, width)`` depth into ``(camera_count, height, width, 4)`` grayscale.
    ...
    """
    camera_index, py, px = wp.tid()
    depth = image[camera_index, py, px]
    if depth <= 0.0:
        out[camera_index, py, px, 0] = wp.uint8(0)
        out[camera_index, py, px, 1] = wp.uint8(0)
        out[camera_index, py, px, 2] = wp.uint8(0)
        out[camera_index, py, px, 3] = wp.uint8(255)
        return
    near = depth_range[0]
    far = depth_range[1]
    denom = wp.max(far - near, 1e-6)
    t = wp.clamp((depth - near) / denom, 0.0, 1.0)
    # Closer -> brighter: near=255, far=50.
    v = wp.uint8(wp.int32((1.0 - t) * 205.0 + 50.0))
    out[camera_index, py, px, 0] = v
    out[camera_index, py, px, 1] = v
    out[camera_index, py, px, 2] = v
    out[camera_index, py, px, 3] = wp.uint8(255)
...
def unpack_shape_index_hash_to_rgba_kernel(
    ...
    out[camera_index, py, px, 0] = wp.uint8((h >> wp.uint32(16)) & wp.uint32(0xFF))
    out[camera_index, py, px, 1] = wp.uint8((h >> wp.uint32(8)) & wp.uint32(0xFF))
    out[camera_index, py, px, 2] = wp.uint8(h & wp.uint32(0xFF))
    out[camera_index, py, px, 3] = wp.uint8(255)
...
def colorize_shape_index_with_palette_kernel(
    ...
    if idx >= num:
        ...
        return
    i = wp.int32(idx)
    out[camera_index, py, px, 0] = colors[i, 0]
    out[camera_index, py, px, 1] = colors[i, 1]
    out[camera_index, py, px, 2] = colors[i, 2]
    out[camera_index, py, px, 3] = wp.uint8(255)
...
class Utils:
    ...
    def create_color_image_output(self, camera_count: int, width: int, height: int) -> wp.array3d[wp.uint32]:
        """Create a color output array for :meth:`~newton.sensors.SensorBatchedCamera.update`.

        Args:
            camera_count: Number of rendered views.
            width: Image width [px].
            height: Image height [px].

        Returns:
            Array of shape ``(camera_count, height, width)``, dtype ``uint32``.
        """
        return wp.zeros((camera_count, height, width), dtype=wp.uint32, device=self.__render_context.device)

    def create_depth_image_output(self, camera_count: int, width: int, height: int) -> wp.array3d[wp.float32]:
        """Create a depth output array for :meth:`~newton.sensors.SensorBatchedCamera.update`.
        ...
        Returns:
            Array of shape ``(camera_count, height, width)``, dtype ``float32``.
        """
        return wp.zeros((camera_count, height, width), dtype=wp.float32, device=self.__render_context.device)

    def create_shape_index_image_output(self, camera_count: int, width: int, height: int) -> wp.array3d[wp.uint32]:
        """Create a shape-index output array for :meth:`~newton.sensors.SensorBatchedCamera.update`.
    ...
    def create_normal_image_output(self, camera_count: int, width: int, height: int) -> wp.array3d[wp.vec3f]:
        """Create a normal output array for :meth:`~newton.sensors.SensorBatchedCamera.update`.
        ...
        Returns:
            Array of shape ``(camera_count, height, width)``, dtype ``vec3f``.
        """
        return wp.zeros((camera_count, height, width), dtype=wp.vec3f, device=self.__render_context.device)

    def create_albedo_image_output(self, camera_count: int, width: int, height: int) -> wp.array3d[wp.uint32]:
        """Create an albedo output array for :meth:`~newton.sensors.SensorBatchedCamera.update`.
    ...
    def create_hdr_color_image_output(self, camera_count: int, width: int, height: int) -> wp.array3d[wp.vec3f]:
        """Create a linear HDR color output array for :meth:`~newton.sensors.SensorBatchedCamera.update`.
    ...
    def create_camera_indices(
        ...
        Returns:
            Array of shape ``(view_count, 2)``, dtype ``int32``. Column 0
            contains world indices and column 1 contains ray-bundle indices.
        """
    ...
    def convert_ray_depth_to_forward_depth(
        ...
        Args:
            depth_image: Ray-distance depth [m] from
                :meth:`~newton.sensors.SensorBatchedCamera.update`, shape
                ``(camera_count, height, width)``.
            ...
            camera_indices: View mapping, shape ``(camera_count, 2)``.
                Column 1 selects the ray bundle from *camera_rays*, matching
                :meth:`~newton.sensors.SensorBatchedCamera.update`.
        ...
        """
        camera_count, height, width = depth_image.shape
    ...
    def flatten_color_image_to_rgba(
        ...
        cameras_per_row: int | None = None,
    ) -> wp.array3d[wp.uint8]:
        """Flatten rendered color image to a tiled RGBA buffer.

        Arranges ``camera_count`` rendered-view tiles in a grid.
        ...
        Args:
            image: Color output from :meth:`~newton.sensors.SensorBatchedCamera.update`, shape
                ``(camera_count, height, width)``.
            out_buffer: Pre-allocated RGBA buffer. If None, allocates a new one.
            cameras_per_row: Tiles per row in the grid. If None, picks a square-ish layout.
        """
        camera_count, height, width = image.shape

        out_buffer, cameras_per_row = self.__reshape_buffer_for_flatten(
            width, height, camera_count, out_buffer, cameras_per_row
        )
    ...
    def to_rgba_from_color(
        ...
        Args:
            image: Color sensor output, shape
                ``(camera_count, H, W)``, dtype ``uint32``
                (packed RGBA: ``R | G<<8 | B<<16 | A<<24``). Must be
                contiguous; arrays returned by
                :meth:`~newton.sensors.SensorBatchedCamera.update` always satisfy this.

        Returns:
            Array of shape ``(camera_count, H, W, 4)``,
            dtype ``uint8``, aliasing *image*.
        """
        camera_count, height, width = image.shape
        return image.view(wp.vec4ub).reshape((camera_count, height, width)).view(wp.uint8)
    ...
    def to_rgba_from_normal(
        ...
        Args:
            image: Normal output, shape ``(camera_count, H, W)``,
                dtype ``vec3f``.
            out_buffer: Optional pre-allocated output of shape
                ``(camera_count, H, W, 4)``, dtype ``uint8``.

        Returns:
            Array of shape ``(camera_count, H, W, 4)``, dtype
            ``uint8``. Suitable for :meth:`~newton.viewer.ViewerBase.log_image`.
        """
        camera_count, height, width = image.shape

        if out_buffer is None:
            out_buffer = wp.empty((camera_count, height, width, 4), dtype=wp.uint8, device=self.__render_context.device)
        else:
            _validate_rgba_out_buffer("to_rgba_from_normal", out_buffer, (camera_count, height, width, 4), image.device)
    ...
    def to_rgba_from_depth(
        ...
        Args:
            image: Depth output, shape ``(camera_count, H, W)``,
        ...
        if out_buffer is None:
            out_buffer = wp.empty((camera_count, height, width, 4), dtype=wp.uint8, device=device)
        else:
            _validate_rgba_out_buffer("to_rgba_from_depth", out_buffer, (camera_count, height, width, 4), image.device)
    ...
    def to_rgba_from_shape_index(
        ...
        if out_buffer is None:
            ...
            _validate_rgba_out_buffer(
                "to_rgba_from_shape_index", out_buffer, (camera_count, height, width, 4), image.device
            )
    ...
    def flatten_normal_image_to_rgba(
        ...
        Args:
            image: Normal output from :meth:`~newton.sensors.SensorBatchedCamera.update`, shape
                ``(camera_count, height, width)``.
            out_buffer: Pre-allocated RGBA buffer. If None, allocates a new one.
            cameras_per_row: Tiles per row in the grid. If None, picks a square-ish layout.
        """
    ...
    def flatten_depth_image_to_rgba(
        ...
        Args:
            image: Depth output from :meth:`~newton.sensors.SensorBatchedCamera.update`, shape
                ``(camera_count, height, width)``.
            out_buffer: Pre-allocated RGBA buffer. If None, allocates a new one.
            cameras_per_row: Tiles per row in the grid. If None, picks a square-ish layout.
    ...
    def assign_checkerboard_material_to_all_shapes(self, resolution: int = 64, checker_size: int = 32):
        ...
        """
        warnings.warn(
            "``SensorBatchedCamera.utils.assign_checkerboard_material_to_all_shapes`` is deprecated as of Newton 1.4. "
            "Use ``SensorBatchedCamera.utils.assign_checkerboard_material(shape_indices=...)`` instead.",
    ...
    def __reshape_buffer_for_flatten(
        ...
        if cameras_per_row is None:
            cameras_per_row = math.ceil(math.sqrt(camera_count))
        elif cameras_per_row < 1:
            raise ValueError(f"cameras_per_row must be >= 1, got {cameras_per_row}")
        cameras_per_col = math.ceil(camera_count / cameras_per_row)

        if out_buffer is None:
            return wp.empty(
                (
                    cameras_per_col * height,
                    cameras_per_row * width,
                    4,
                ),
                dtype=wp.uint8,
                device=self.__render_context.device,
        ...
        return out_buffer.reshape((cameras_per_col * height, cameras_per_row * width, 4)), cameras_per_row
