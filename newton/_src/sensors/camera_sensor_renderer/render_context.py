...
class RenderContext:
    ...
    def render(
        ...
        *,
        ...
        world_enabled: wp.array[wp.bool],
        ...
        Args:
            ...
            camera_rays: Ray origins and directions, shape
                ``(height, width, 2)``.
            world_enabled: Per-world render mask, shape ``(world_count,)``.
        ...
        if has_shapes or has_particles or self.has_triangle_mesh or self.has_gaussians:
            ...
            assert world_enabled.shape == (self.world_count,), f"world_enabled size must match {self.world_count}"
            assert world_enabled.dtype == wp.bool, f"world_enabled dtype must be bool, got {world_enabled.dtype}"
            assert world_enabled.device == wp.get_device(self.device), (
                f"world_enabled device must match {wp.get_device(self.device)}"
            )

            if color_image is not None:
                assert color_image.shape == (self.world_count, height, width), (
                    f"color_image size must match {self.world_count} x {height} x {width}"
                )

            if depth_image is not None:
                assert depth_image.shape == (self.world_count, height, width), (
                    f"depth_image size must match {self.world_count} x {height} x {width}"
                )

            if forward_depth_image is not None:
                assert forward_depth_image.shape == (self.world_count, height, width), (
                    f"forward_depth_image size must match {self.world_count} x {height} x {width}"
                )

            if shape_index_image is not None:
                assert shape_index_image.shape == (self.world_count, height, width), (
                    f"shape_index_image size must match {self.world_count} x {height} x {width}"
                )

            if normal_image is not None:
                assert normal_image.shape == (self.world_count, height, width), (
                    f"normal_image size must match {self.world_count} x {height} x {width}"
                )

            if albedo_image is not None:
                assert albedo_image.shape == (self.world_count, height, width), (
                    f"albedo_image size must match {self.world_count} x {height} x {width}"
                )
            if hdr_color_image is not None:
                assert hdr_color_image.shape == (self.world_count, height, width), (
                    f"hdr_color_image size must match {self.world_count} x {height} x {width}"
                )

            total_pixels = self.world_count * width * height
            ...
            wp.launch(
                ...
                dim=(self.world_count * pixels_per_view),
                inputs=[
                    # Model and config
                    self.world_count,
                    self.light_count,
