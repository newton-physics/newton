...
class RenderContext:
    ...
    def render(
        ...
        *,
        ...
        world_render_flags: wp.array[wp.int32],
        ...
        if has_shapes or has_particles or self.has_triangle_mesh or self.has_gaussians:
            ...
            assert world_render_flags.shape == (self.world_count,), (
                f"world_render_flags size must match {self.world_count}"
            )
            assert world_render_flags.dtype == wp.int32, (
                f"world_render_flags dtype must be int32, got {world_render_flags.dtype}"
            )
            assert world_render_flags.device == wp.get_device(self.device), (
                f"world_render_flags device must match {wp.get_device(self.device)}"
            )
