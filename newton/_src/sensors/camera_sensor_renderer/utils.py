...
class Utils:
    ...
    def create_color_image_output(self, width: int, height: int) -> wp.array3d[wp.uint32]:
        """Create a color output array for camera sensor rendering.
        ...
        Returns:
            Array of shape ``(world_count, height, width)``, dtype ``uint32``.
        """
        return wp.zeros(
            (self.__render_context.world_count, height, width),
            dtype=wp.uint32,
            device=self.__render_context.device,
        )

    def create_depth_image_output(self, width: int, height: int) -> wp.array3d[wp.float32]:
        """Create a depth output array for camera sensor rendering.
        ...
        Returns:
            Array of shape ``(world_count, height, width)``, dtype ``float32``.
        """
        return wp.zeros(
            (self.__render_context.world_count, height, width),
            dtype=wp.float32,
    ...
    def create_forward_depth_image_output(self, width: int, height: int) -> wp.array3d[wp.float32]:
        """Create a forward-depth output array for camera sensor rendering.
        ...
        Returns:
            Array of shape ``(world_count, height, width)``, dtype ``float32``.
        """
        return self.create_depth_image_output(width, height)

    def create_shape_index_image_output(self, width: int, height: int) -> wp.array3d[wp.uint32]:
        """Create a shape-index output array for camera sensor rendering.
    ...
    def create_normal_image_output(self, width: int, height: int) -> wp.array3d[wp.vec3f]:
        """Create a normal output array for camera sensor rendering.
        ...
        Returns:
            Array of shape ``(world_count, height, width)``, dtype ``vec3f``.
        """
        return wp.zeros(
            (self.__render_context.world_count, height, width),
            dtype=wp.vec3f,
    ...
    def create_albedo_image_output(self, width: int, height: int) -> wp.array3d[wp.uint32]:
        """Create an albedo output array for camera sensor rendering.
    ...
    def create_hdr_color_image_output(self, width: int, height: int) -> wp.array3d[wp.vec3f]:
        """Create a linear HDR color output array for camera sensor rendering.
    ...
    def flatten_color_image_to_rgba(
        ...
        Arranges one tile per world in a grid.
        ...
        Args:
            image: Color output from :meth:`~newton.sensors.CameraSensor.update`, shape ``(world_count, height, width)``.
    ...
    def to_rgba_from_color(
        ...
        Args:
            image: Color sensor output, shape
                ``(world_count, H, W)``, dtype ``uint32``
                (packed RGBA: ``R | G<<8 | B<<16 | A<<24``). Must be
                contiguous.

        Returns:
            Array of shape ``(world_count, H, W, 4)``,
            dtype ``uint8``, aliasing *image*.
        """
        world_count, h, w = image.shape
        return image.view(wp.vec4ub).reshape((world_count, h, w)).view(wp.uint8)
    ...
    def to_rgba_from_normal(
        ...
        Args:
            image: Normal output, shape ``(world_count, H, W)``, dtype ``vec3f``.
            out_buffer: Optional pre-allocated output of shape
                ``(world_count, H, W, 4)``, dtype ``uint8``.

        Returns:
            Array of shape ``(world_count, H, W, 4)``, dtype
        ...
        if out_buffer is None:
            out_buffer = wp.empty((world_count, h, w, 4), dtype=wp.uint8, device=self.__render_context.device)
        else:
            _validate_rgba_out_buffer("to_rgba_from_normal", out_buffer, (world_count, h, w, 4), image.device)
    ...
    def to_rgba_from_depth(
        ...
        Args:
            image: Depth output, shape ``(world_count, H, W)``, dtype
        ...
        if out_buffer is None:
            out_buffer = wp.empty((world_count, h, w, 4), dtype=wp.uint8, device=device)
        else:
            _validate_rgba_out_buffer("to_rgba_from_depth", out_buffer, (world_count, h, w, 4), image.device)
    ...
    def to_rgba_from_shape_index(
        ...
        if out_buffer is None:
            ...
            _validate_rgba_out_buffer("to_rgba_from_shape_index", out_buffer, (world_count, h, w, 4), image.device)
    ...
    def flatten_normal_image_to_rgba(
        ...
        Args:
            image: Normal output from :meth:`~newton.sensors.CameraSensor.update`, shape ``(world_count, height, width)``.
            out_buffer: Pre-allocated RGBA buffer. If None, allocates a new one.
            worlds_per_row: Tiles per row in the grid. If None, picks a square-ish layout.
        """
        height = image.shape[1]
    ...
    def flatten_depth_image_to_rgba(
        ...
        Args:
            image: Depth output from :meth:`~newton.sensors.CameraSensor.update`, shape ``(world_count, height, width)``.
            out_buffer: Pre-allocated RGBA buffer. If None, allocates a new one.
            worlds_per_row: Tiles per row in the grid. If None, picks a square-ish layout.
    ...
    def assign_checkerboard_material(
        ...
        *,
        ...
        self.__render_context._texture_data_source = [checkerboard_data]
        self.__render_context.texture_data = wp.array(
            self.__render_context._texture_data_source, dtype=TextureData, device=self.__render_context.device
        )
    ...
    def __reshape_buffer_for_flatten(
        ...
        if worlds_per_row is None:
            worlds_per_row = math.ceil(math.sqrt(self.__render_context.world_count))
        elif worlds_per_row < 1:
            raise ValueError(f"worlds_per_row must be >= 1, got {worlds_per_row}")
        worlds_per_col = math.ceil(self.__render_context.world_count / worlds_per_row)
