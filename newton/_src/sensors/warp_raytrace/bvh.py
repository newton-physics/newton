...
@wp.func
def compute_box_bounds(pos: wp.vec3, rot: wp.mat33, size: wp.vec3) -> tuple[wp.vec3, wp.vec3]:
    min_bound = wp.vec3(wp.inf, wp.inf, wp.inf)
    max_bound = wp.vec3(-wp.inf, -wp.inf, -wp.inf)
...
@wp.func
def compute_sphere_bounds(pos: wp.vec3, rot: wp.mat33, size: wp.vec3) -> tuple[wp.vec3, wp.vec3]:
    radius = size[0]
    return pos - wp.vec3(radius, radius, radius), pos + wp.vec3(radius, radius, radius)


@wp.func
def compute_capsule_bounds(pos: wp.vec3, rot: wp.mat33, size: wp.vec3) -> tuple[wp.vec3, wp.vec3]:
    radius = size[0]
    half_length = size[1]
...
@wp.func
def compute_cone_bounds(pos: wp.vec3, rot: wp.mat33, size: wp.vec3) -> tuple[wp.vec3, wp.vec3]:
    radius = size[0]
    half_length = size[1]
    local_end1 = wp.vec3(0.0, 0.0, -half_length)
    local_end2 = wp.vec3(0.0, 0.0, half_length)
    ...
    inflate = wp.vec3(radius, radius, 0.0)
...
@wp.func
def compute_plane_bounds(pos: wp.vec3, rot: wp.mat33, size: wp.vec3) -> tuple[wp.vec3, wp.vec3]:
...
def compute_bvh_bounds(
    num_geom_in_bvh: int,
    num_worlds: int,
    geom_world_index: wp.array(dtype=int),
    ...
    mesh_bounds: wp.array2d(dtype=wp.vec3),
    ...
    lowers[world_id * num_geom_in_bvh + bvh_geom_local] = lower
    uppers[world_id * num_geom_in_bvh + bvh_geom_local] = upper
