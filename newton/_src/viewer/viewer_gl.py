...
# camera -> GL-style view/projection (column-major semantics for ImGuizmo)
def _camera_view_proj_gl(viewer) -> tuple[np.ndarray, np.ndarray]:
    cam = viewer.camera
    # Build view from camera basis and position
    r = np.asarray(cam.get_right(), dtype=np.float32)
    u = np.asarray(cam.get_up(), dtype=np.float32)
    f = np.asarray(cam.get_front(), dtype=np.float32)
    ...
    r, u, f = _n(r), _n(u), _n(f)
    V = np.eye(4, dtype=np.float32)
    V[0, :3], V[1, :3], V[2, :3] = r, u, -f
    pos = np.array([cam.pos[0], cam.pos[1], cam.pos[2]], dtype=np.float32)
    V[:3, 3] = -V[:3, :3] @ pos  # translate
    view = V.astype(np.float32)  # column-major for ImGuizmo

    # Projection (OpenGL-style, z in [-1,1])
    io = viewer.ui.io
    aspect = max(1e-6, float(io.display_size[0]) / max(1.0, float(io.display_size[1])))
    fovy = np.deg2rad(float(cam.fov))
    n, f_ = 0.05, 500.0
    ...
    P[2, 3] = (2.0 * f_ * n) / (n - f_)
    P[3, 2] = -1.0
    proj = P.astype(np.float32)  # column-major for ImGuizmo
    return view, proj
...
class ViewerGL(ViewerBase):
    ...
    def _render_gizmos(self):
        ...
        # Camera matrices (column-major float32)
        view, proj = _camera_view_proj_gl(self)
