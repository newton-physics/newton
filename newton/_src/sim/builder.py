...
class ModelBuilder:
    ...
    class ShapeConfig:
        ...
        """Collision group ID. Defaults to 1 (default group).

        Set to 0 to disable group-derived automatic collisions when ``collision_type``
        and ``collision_affinity`` are not authored. If either explicit mask is set,
        runtime filtering uses the authored masks instead.
    ...
    def _compile_shape_collision_masks(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        unique_groups = sorted(
            {
                group
                for group, collision_type, collision_affinity in zip(
                    self.shape_collision_group,
                    self.shape_collision_type,
                    self.shape_collision_affinity,
                    strict=True,
                )
                if group != 0 and collision_type is None and collision_affinity is None
            }
        )
