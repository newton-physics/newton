...
def _apply_steepest_descent(
    # inputs
    joint_q: wp.array2d(dtype=wp.float32),  # (n_batch, n_dofs)
    gradient: wp.array2d(dtype=wp.float32),  # (n_batch, n_dofs)
    step_size: float,  # scalar
    # outputs
    joint_q_out: wp.array2d(dtype=wp.float32),  # (n_batch, n_dofs)
):
    row, coord_idx = wp.tid()
    joint_q_out[row, coord_idx] = joint_q[row, coord_idx] - step_size * gradient[row, coord_idx]
...
class IKLBFGSOptimizer:
    ...
    * **MIXED**      — analytic where available, autodiff fallback elsewhere
    ...
    def _alloc_solver_buffers(self, grad: bool):
        ...
        self.joint_q_prev = wp.zeros((self.n_batch, self.n_coords), dtype=wp.float32, device=device)
    ...
    def _grad_autodiff(self, ctx: BatchCtx, out_grad: wp.array2d):
        ...
        with self.tape:
            ...
            residuals_flat_1d = ctx.residuals.flatten()

        self.tape.outputs = [residuals_flat_1d]
        ...
        if ctx.autodiff_mask is not None and ctx.autodiff_seed is not None:
            ...
            seed_flat = ctx.autodiff_seed.flatten()
            self.tape.backward(grads={residuals_flat_1d: seed_flat})
        else:
            self.tape.backward(grads={residuals_flat_1d: residuals_flat_1d})
    ...
    def _step(self, joint_q, iteration=0):
        ...
        if iteration == 0:
            ...
            wp.copy(self.joint_q_prev, joint_q)
            wp.launch(
                _apply_steepest_descent,
                dim=[self.n_batch, self.n_coords],
                inputs=[joint_q, self.gradient, 1e-2],
                outputs=[joint_q],
                device=self.device,
            )
            return

        self._update_history(joint_q)
    ...
    def _update_history(self, joint_q):
    ...
    def _build_specialised(cls, key):
        ...
        def _update_history_template(
            # inputs
            joint_q: wp.array2d(dtype=wp.float32),  # (n_batch, n_dofs)
            joint_q_prev: wp.array2d(dtype=wp.float32),  # (n_batch, n_dofs)
            gradient: wp.array2d(dtype=wp.float32),  # (n_batch, n_dofs)
            ...
            q_curr = wp.tile_load(joint_q[row], shape=(DOF,))
            q_prev = wp.tile_load(joint_q_prev[row], shape=(DOF,))
        ...
        def _generate_candidates_template(
            ...
            # outputs
            candidate_q: wp.array3d(dtype=wp.float32),  # (n_batch, n_line_steps, n_dofs)
        ):
            row, step_idx = wp.tid()
            DOF = _Specialised.TILE_N_DOFS
            ...
            wp.tile_store(candidate_q[row, step_idx], q_new)
        ...
        def _select_best_step_template(
            ...
            best_step_idx_out[row] = best_idx

            best_q = wp.tile_load(candidate_q[row, best_idx], shape=(DOF,), storage="shared")
            wp.tile_store(joint_q_out[row], best_q)
        ...
        _compute_slope_candidates_tiled.__name__ = f"_compute_slope_candidates_tiled_{C}_{N_LINE_SEARCH}"
        _compute_slope_candidates_tiled.__qualname__ = f"_compute_slope_candidates_tiled_{C}_{N_LINE_SEARCH}"
        ...
        class _Specialised(IKLBFGSOptimizer):
        ...
        _Specialised._compute_slope_candidates_tiled = staticmethod(_compute_slope_candidates_tiled)
