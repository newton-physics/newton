...
or ``(P J)^T`` value copy is materialized; the only auxiliary data are ``int32`` index
...
from ..core.types import float32, int32, mat33f, vec6f
...
def warp_subreduce_sum(v: float32, width: int32) -> float32: ...
...
def make_row_index_kernel(max_blocks: int):
    ...
    slots. This is an ``int32`` index into the Jacobian's ``nzb_values`` -- no
    ...
    def row_index_kernel(
        ...
        row_offset: wp.array[int32],
        # Outputs:
        row_blk: wp.array2d[int32],
        slot_count: wp.array[int32],
    ):
        wid, bi = wp.tid()
        if bi >= num_nzb[wid]:
            return
...
def make_fill_sort_kernel():
    ...
    def fill_sort_kernel(
        ...
        nzb_coords: wp.array2d[int32],
        # Outputs:
        sort_key: wp.array[int32],
        sort_val: wp.array[int32],
        seg_end: wp.array[int32],
    ):
        wid, bi = wp.tid()
        if bi == 0:
...
def build_row_index(
    *,
    ...
    row_blk = out_row_blk if out_row_blk is not None else wp.empty((total_rows, max_blocks), dtype=int32, device=device)
    slot_count = out_slot_count if out_slot_count is not None else wp.empty((total_rows,), dtype=int32, device=device)
    row_blk.fill_(-1)
    slot_count.zero_()
    wp.launch(
...
def build_transpose_index(
    *,
    ...
    sort_key = out_sort_key if out_sort_key is not None else wp.empty((alloc,), dtype=int32, device=device)
    sort_val = out_sort_val if out_sort_val is not None else wp.empty((alloc,), dtype=int32, device=device)
    seg_end = out_seg_end if out_seg_end is not None else wp.empty((n_worlds,), dtype=int32, device=device)
    cursor = out_cursor if out_cursor is not None else wp.empty((n_worlds, max_major_cols), dtype=int32, device=device)
    cursor.fill_(-1)
...
def make_fused_cr_kernel(max_rows: int, max_cols: int, max_blocks: int, block_dim: int = 0):
    ...
    while T * 2 <= 32 and NB * (T * 2) <= BD:
        T *= 2
    vecNR = wp.types.vector(length=NR, dtype=float32)

    @wp.func
    def _minv(im: float32, iI: mat33f, tv: vec6f) -> vec6f:
    ...
    @wp.func
    def _rsum(x: float32) -> float32:
        # Reduce a per-thread CR scalar across the block with a cooperative tile_sum.
        return wp.tile_sum(wp.tile(x))[0]

    @wp.func
    def _rsum2(a: float32, b: float32) -> wp.vec2:
        # Reduce two co-located CR scalars in one pass: build a (2, block_dim) tile from the
        # per-thread vec2 and reduce along the thread axis with a single cooperative tile_sum
        # -- one cross-warp barrier for both sums instead of two.
    ...
    def _apply_A(
        ...
        row_idx_sorted: wp.array[int32],
        cursor: wp.array2d[int32],
        inv_m: wp.array[float32],
        inv_I: wp.array[mat33f],
        bodies_offset: wp.array[int32],
    ) -> vecNR:
        ...
        wp.tile_scatter_add(st, bidx, vec6f(q0, q1, q2, q3, q4, q5), body < nbod and sub == 0, False)
        ...
        for i in range(wp.static(NR)):
            ...
            if row < n:
                ...
                for k in range(wp.static(MB)):
                    gb = row_blk[roff + row, k]
                    if gb >= 0:
                        col = nzb_coords[gb, 1]
                        bl = col // 6
                        body_g = bo + bl
                        tv = st[bl]  # the body's 6 transpose outputs (one vec6)
                        acc += wp.dot(nzb_values[gb], _minv(inv_m[body_g], inv_I[body_g], tv))
    ...
    def fused_cr_kernel(
        ...
        maxiter: wp.array[int32],
        atol: wp.array[float32],
        rtol: wp.array[float32],
        # Outputs:
        out_iters: wp.array[int32],
        out_resid: wp.array[float32],
    ):
        w, t = wp.tid()
        if not world_active[w]:
            if t == 0:
                out_iters[w] = 0
                out_resid[w] = float32(0.0)
            return
        ...
        at = atol[w]
        rt = rtol[w]
        atol_sq = wp.max(rt * rt * _rsum(bb), at * at)
        ...
        rAr_local = float32(0.0)
        rnorm_local = float32(0.0)
        for i in range(wp.static(NR)):
            if t + i * BD >= n:
                break
            rAr_local += r_rowv[i] * ar_rowv[i]
            rnorm_local += r_rowv[i] * r_rowv[i]
        rr0 = _rsum2(rAr_local, rnorm_local)
        rAr = rr0[0]
        r_norm_sq = rr0[1]
        ...
        for _it in range(mi):
            ...
            denom_local = float32(0.0)
            for i in range(wp.static(NR)):
                if t + i * BD >= n:
                    break
                denom_local += ap_rowv[i] * ap_rowv[i]
            denom = _rsum(denom_local)
            alpha = float32(0.0)
            ...
            for i in range(wp.static(NR)):
                ...
                rnorm_local += r_rowv[i] * r_rowv[i]
            rr = _rsum2(rAr_new_local, rnorm_local)
            rAr_new = rr[0]
            r_norm_sq = rr[1]
            beta = float32(0.0)
            if rAr > float32(0.0):
