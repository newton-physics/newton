...
class ConjugateResidualSolverFused(IterativeSolver):
    ...
    def _allocate_impl(self, operator, **kwargs: dict[str, Any]) -> None:
        ...
        with wp.ScopedDevice(device):
            self._row_blk = wp.empty((self._total_rows, MAX_BLOCKS_PER_ROW), dtype=int32)
            self._slot_count = wp.empty((self._total_rows,), dtype=int32)
            self._sort_key = wp.empty((max(2 * self._total_nnz, 2),), dtype=int32)
            self._sort_val = wp.empty((max(2 * self._total_nnz, 2),), dtype=int32)
            ...
            self._row_idx_sorted = wp.empty((max(2 * self._total_nnz, 2),), dtype=int32)
            self._seg_end = wp.empty((n_worlds,), dtype=int32)
            self._cursor = wp.empty((n_worlds, self._max_major_cols), dtype=int32)
            self._iters = wp.zeros((n_worlds,), dtype=int32)
            self._resid = wp.zeros((n_worlds,), dtype=float32)
            bodies_offset = model.info.bodies_offset.numpy()
            self._nbd = wp.array((np.diff(bodies_offset) * 6).astype(np.int32), dtype=int32)
            self._precond_dummy = wp.zeros((1,), dtype=float32)
            # Full-length zero eta fallback (the fused kernel reads eta[voff + row] for every active row).
            self._eta_dummy = wp.zeros((self._total_rows,), dtype=float32)
        ...
        self._atol_arr = wp.full(n_worlds, 1e-8, dtype=float32, device=device)
        self._rtol_arr = wp.full(n_worlds, 1e-8, dtype=float32, device=device)
