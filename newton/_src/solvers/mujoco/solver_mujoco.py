    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            use_mujoco_contacts: If True, use the MuJoCo contact solver. If False, use the Newton contact solver (newton contacts must be passed in through the step function in that case). When Newton contacts are used with elliptic friction cones, per-shape :attr:`~newton.Model.shape_material_kf` is mapped to per-contact ``solreffriction``, targeting the force-space friction slope ``f = -kf * v`` below the Coulomb limit; set ``kf`` to ``0`` to inherit ``solref`` instead. This mapping applies regardless of the shape's ``solref_mode``. Exactness and saturation caveats: see :ref:`mujoco-contact-friction-solreffriction`.
