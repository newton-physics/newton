...
class SolverMuJoCo(SolverBase):
    ...
    def _convert_to_mjc(
        ...
        *,
        ...
        selected_shapes_set = {int(shape) for shape in selected_shapes}
        shape_collision_filter_pairs = set(model.shape_collision_filter_pairs)
        mujoco_attrs = getattr(model, "mujoco", None)

        def _shape_worlds_compatible(shape_a: int, shape_b: int) -> bool:
            return (
                int(shape_world[shape_a]) == -1
                or int(shape_world[shape_b]) == -1
                or int(shape_world[shape_a]) == int(shape_world[shape_b])
            )

        def _shape_masks_compatible(shape_a: int, shape_b: int) -> bool:
            return (int(shape_collision_type[shape_a]) & int(shape_collision_affinity[shape_b]) != 0) or (
                int(shape_collision_type[shape_b]) & int(shape_collision_affinity[shape_a]) != 0
            )

        def _mujoco_mask_value(mask: int) -> int:
            return np.array(mask, dtype=np.uint32).view(np.int32).item()
        ...
        for i, shape_a_value in enumerate(colliding_shapes):
            shape_a = int(shape_a_value)
            for shape_b_value in colliding_shapes[i + 1 :]:
                shape_b = int(shape_b_value)
        ...
        def add_geoms(newton_body_id: int):
            ...
            for shape in shapes:
                ...
                if stype == GeoType.HFIELD:
                    ...
                    uses_mujoco_contacts = shape in mujoco_auto_contact_shapes or shape in mujoco_pair_contact_shapes
                ...
                if not (shape_flags[shape] & ShapeFlags.COLLIDE_SHAPES):
                    # Non-colliding shapes can still be exported for visuals,
                    # tendons, actuators, or explicit pair references.
                    geom_params["contype"] = 0
                    geom_params["conaffinity"] = 0
                else:
                    geom_params["contype"] = _mujoco_mask_value(int(shape_collision_type[shape]))
                    geom_params["conaffinity"] = _mujoco_mask_value(int(shape_collision_affinity[shape]))
