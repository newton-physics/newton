...
def convert_warp_coords_to_mj_kernel(
    ...
    if jtype == JointType.FREE:
        # Compose the body's world pose from Newton joint_q (the relative transform
        # between joint anchors) for MuJoCo qpos:
        #   world_body_pose = joint_X_p * joint_q_transform * inv(joint_X_c).
        # joint_qd[0:6] follows the parent-frame contract from State.joint_qd. MuJoCo
        # only allows FREE joints at the worldbody root, so X_wpj == joint_X_p.
        j_tf = wp.transform(
