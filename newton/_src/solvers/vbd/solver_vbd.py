...
class SolverVBD(SolverBase):
    ...
    **soft mode**. The hard/soft mode can be changed per slot via :meth:`set_joint_constraint_mode`.
    ...
    def __init__(
        ...
        if model.body_count > 0 and model.joint_count > 0 and not _vbd_migration_warned["value"]:
            joint_type = model.joint_type
            joint_type_cpu = joint_type.to("cpu") if hasattr(joint_type, "to") else joint_type
            joint_type_np = joint_type_cpu.numpy() if hasattr(joint_type_cpu, "numpy") else np.asarray(joint_type_cpu)
            if bool(np.any(joint_type_np == int(JointType.CABLE))):
                warnings.warn(
                    "Cable rod body-frame convention generated by ModelBuilder.add_rod() and add_rod_graph() "
                    "changed: each capsule body origin now coincides with its capsule COM. Migration: update "
                    "custom rod endpoint anchors to use midpoint-relative coordinates.",
                    UserWarning,
                    stacklevel=2,
                )
