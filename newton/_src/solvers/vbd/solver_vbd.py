    ...
    def __init__(
        ...
        if model.body_count > 0 and not integrate_with_external_rigid_solver and not _vbd_migration_warned["value"]:
            warnings.warn(
                ...
                "SolverVBD.register_custom_attributes(builder) before adding "
                "joints, then pass custom_attributes={'vbd:joint_is_hard': 0} to the relevant add_joint_* "
                "calls. Rebuild models finalized without VBD custom attributes. The optional "
                "dahl_defaults_enabled=False argument only suppresses the separate Dahl friction "
