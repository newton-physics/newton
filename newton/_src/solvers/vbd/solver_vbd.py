    ...
    def __init__(
        ...
        if model.body_count > 0 and not integrate_with_external_rigid_solver and not _vbd_migration_warned["value"]:
            warnings.warn(
                ...
                "add_joint_* calls. Rebuild models finalized without VBD custom attributes. Because "
                "register_custom_attributes() also registers Dahl cable-friction attributes, call "
                "model.vbd.dahl_eps_max.fill_(0.0) before constructing SolverVBD if you do not want "
                "Dahl friction.\n"
