...
@wp.kernel
def accumulate_articulation_external_joints_per_body(
    dt: float,
    ...
    """Accumulate diagonal terms from joints excluded from sparse articulation ranges."""
    body_index = body_ids[wp.tid()]
    force = body_forces[body_index]
    torque = body_torques[body_index]
    H_ll = body_hessian_ll[body_index]
    H_al = body_hessian_al[body_index]
    H_aa = body_hessian_aa[body_index]
    ...
    for joint_counter in range(num_adj_joints):
        ...
        if joint_articulation_sparse[joint_idx] < 0:
            ...
            force = force + joint_force
            torque = torque + joint_torque
            H_ll = H_ll + joint_H_ll
            H_al = H_al + joint_H_al
            H_aa = H_aa + joint_H_aa

    body_forces[body_index] = force
    body_torques[body_index] = torque
    body_hessian_ll[body_index] = H_ll
    body_hessian_al[body_index] = H_al
    body_hessian_aa[body_index] = H_aa
