    ...
    * **MIXED**      — analytic where available, autodiff fallback elsewhere
    ...
    def _build_specialised(cls, key):
        ...
        def _integrate_dq_dof(
            ...
            joint_count = len(joint_type)
            problem_idx = tid // joint_count
            joint_idx = tid % joint_count
...
def _pos_jac_analytic(
    ...
    global_dof = wp.tid()
    problem_idx = global_dof // n_dofs
    dof_idx = global_dof % n_dofs
...
def _limit_residuals(
    ...
    g_dof = wp.tid()
    problem = g_dof // n_dofs
    dof_idx = g_dof % n_dofs
