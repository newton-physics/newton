...
class Example:
    def __init__(self, viewer, num_envs=4, disable_tendons=True, mass_scale=1.0):
        ...
        joint_ordering = [
            "rootJoint", "robot0_forearm", "robot0_WRJ1", "robot0_WRJ0",
        ...
        if self.model.tendon_count > 0:
            ...
            tendon_start = self.model.tendon_start.numpy() if hasattr(self.model.tendon_start, 'numpy') else self.model.tendon_start
            tendon_key = self.model.tendon_key if hasattr(self.model, 'tendon_key') else []
            
            tendon_params = self.model.tendon_params.numpy() if hasattr(self.model.tendon_params, 'numpy') else self.model.tendon_params
        ...
        body_masses = self.model.body_inv_mass.numpy() if hasattr(self.model.body_inv_mass, 'numpy') else self.model.body_inv_mass
        body_inertias = self.model.body_inv_inertia.numpy() if hasattr(self.model.body_inv_inertia, 'numpy') else self.model.body_inv_inertia
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mass-scale", type=float, default=1.0, help="Scale all masses by this factor (try 10 or 100 for stability)")
