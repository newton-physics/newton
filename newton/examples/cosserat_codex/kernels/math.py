...
This module contains quaternion operations, matrix operations, and
block-tridiagonal helper functions used by the direct solver.
"""
...
def _mat33_solve_upper(L: wp.mat33, b: wp.vec3) -> wp.vec3:
    """Solve L^T * x = b where L is lower triangular (solves upper system)."""
    x2 = b[2] / L[2, 2]
...
def _mat33_cholesky_solve(L: wp.mat33, b: wp.vec3) -> wp.vec3:
    """Solve L * L^T * x = b given Cholesky factor L (lower triangular)."""
...
# ==============================================================================
# Block-tridiagonal indexing and operations
# ==============================================================================
...
@wp.func
def _block_row(blocks: wp.array(dtype=wp.float32), block: int, row: int) -> tuple[wp.vec3, wp.vec3]:
    """Load a row from a 6x6 block as two 3-vectors."""
    base = block * 36 + row * 6
    v0 = wp.vec3(
        blocks[base + 0],
        blocks[base + 1],
