...
"""Warp GPU-resident rod state implementation.

This module provides a fully GPU-resident rod state using Warp arrays,
avoiding host-device transfers during simulation.
"""
...
class WarpResidentRodState(DefKitDirectRodState):
    ...
    Attributes:
        ...
        direct_solve_backend: Which solver backend to use.
        positions_wp: GPU-resident positions.
        velocities_wp: GPU-resident velocities.
    ...
    def _init_warp_state(self):
        """Initialize GPU-resident arrays."""
    ...
    def _sync_from_host_all(self):
        """Synchronize all state from host to device."""
    ...
    def apply_root_translation(self, dx: float, dy: float, dz: float):
        """Apply translation to root particle."""
        if self.num_points == 0:
            return
        wp.launch(
            _warp_apply_root_translation,
    ...
    def apply_root_rotation(self, q: np.ndarray):
        """Apply rotation to root particle."""
        if self.num_points == 0:
            return
    ...
    def orientations_numpy(self) -> np.ndarray:
        """Get orientations as numpy array."""
    ...
    def predict_positions(self, dt: float, linear_damping: float):
        """Predict positions for constraint projection."""
        if self.num_points == 0:
            return
        gravity = wp.vec3(
    ...
    def predict_rotations(self, dt: float, angular_damping: float):
        """Predict rotations for constraint projection."""
        if self.num_points == 0:
            return
        wp.launch(
            _warp_predict_rotations,
