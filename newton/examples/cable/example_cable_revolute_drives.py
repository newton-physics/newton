...
def _extract_revolute_angle(model, body_q_np, joint_index):
    """Extract the free-axis rotation angle for a revolute joint.

    Uses swing-twist decomposition matching the XPBD/VBD convention.
    """
...
class Example:
    """Two cables comparing revolute joint drive with and without limits.

    Each cable hangs from a kinematic sphere at z=4 via a revolute joint (Y-axis):

    - **Drive**: spring-damper (ke=2000, kd=100) tracks +/-1.2 rad oscillation.
    - **Drive+Limit**: same drive but joint limits clamp swing to +/-0.3 rad.
    """
    ...
    def __init__(self, viewer, args=None):
        ...
        # Cable parameters -- same as example_cable_revolute_joints for stability.
        num_segments = 50
        segment_length = 0.05
        cable_radius = 0.015
        ...
        self.drive_amplitude = 1.2  # rad
        self.limit_bound = 0.3  # rad
        ...
        for i in range(2):
            ...
            # Revolute joint -- both share the same anchor geometry.
        ...
        # Resolve revolute joint DOF indices after finalize().
        joint_types = self.model.joint_type.numpy()
        joint_qd_start = self.model.joint_qd_start.numpy()

        rev_indices = [
            i for i in range(self.model.joint_count) if int(joint_types[i]) == int(newton.JointType.REVOLUTE)
        ]
        assert len(rev_indices) == 2, f"Expected 2 revolute joints, found {len(rev_indices)}"
        self._rev_joint_indices = rev_indices

        # Drive target is written to both cables.
        drive_dofs = [int(joint_qd_start[rev_indices[0]]), int(joint_qd_start[rev_indices[1]])]
    ...
    def test_final(self):
        ...
        theta_drive = _extract_revolute_angle(self.model, body_q_np, self._rev_joint_indices[0])
        theta_limit = _extract_revolute_angle(self.model, body_q_np, self._rev_joint_indices[1])

        # Drive cable should track the target.
        if abs(theta_drive - target) > 0.5:
            raise AssertionError(f"Drive tracking: theta={theta_drive:.3f}, target={target:.3f}")
