...
class Example:
    def __init__(self, stage_path=None, headless=False, num_envs=8, use_cuda_graph=True):
        ...
        # Create main builder and add multiple environments
        ...
        self.use_cuda_graph = (
            self.device.is_cuda and wp.is_mempool_enabled(wp.get_device()) and use_cuda_graph
        )
...
if __name__ == "__main__":
    ...
    with wp.ScopedDevice(args.device):
        example = Example(stage_path=args.stage_path, headless=args.headless, num_envs=args.num_envs, use_cuda_graph=args.use_cuda_graph)
