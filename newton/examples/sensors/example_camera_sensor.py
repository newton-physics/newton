...
class Example:
    def __init__(self, viewer: ViewerGL, args):
        ...
        self.camera_sensor.utils.create_default_light(enable_shadows=True)
        self.camera_sensor.utils.assign_checkerboard_material(shape_indices=self.ground_shape_indices)

        self.camera_sensor_color_image = self.camera_sensor.utils.create_color_image_output(
            self.camera_sensor.width, self.camera_sensor.height
        )
        self.camera_sensor_albedo_image = self.camera_sensor.utils.create_albedo_image_output(
            self.camera_sensor.width, self.camera_sensor.height
        )
        self.camera_sensor_depth_image = self.camera_sensor.utils.create_depth_image_output(
            self.camera_sensor.width, self.camera_sensor.height
        )
        self.camera_sensor_normal_image = self.camera_sensor.utils.create_normal_image_output(
            self.camera_sensor.width, self.camera_sensor.height
        )
        self.camera_sensor_shape_index_image = self.camera_sensor.utils.create_shape_index_image_output(
            self.camera_sensor.width, self.camera_sensor.height
        )
