...
###########################################################################
# Example Batched Camera Sensor
#
# Shows how to use the SensorBatchedCamera class and display its output
# via Viewer.log_image.
#
# Command: python -m newton.examples sensor_batched_camera
#
###########################################################################
...
class Example:
    def __init__(self, viewer: ViewerGL, args):
        ...
        self.camera_count = self.world_count_total
        self.sensor_render_width = 256
        self.sensor_render_height = 256

        # Setup Batched Camera Sensor
        self.batched_camera_sensor = SensorBatchedCamera(model=self.model)
        self.batched_camera_sensor.utils.create_default_light(enable_shadows=True)
        self.batched_camera_sensor.utils.assign_checkerboard_material(shape_indices=self.ground_shape_indices)
        ...
        self.batched_camera_sensor_color_image = self.batched_camera_sensor.utils.create_color_image_output(
            self.camera_count, self.sensor_render_width, self.sensor_render_height
        )
        self.batched_camera_sensor_albedo_image = self.batched_camera_sensor.utils.create_albedo_image_output(
            self.camera_count, self.sensor_render_width, self.sensor_render_height
        )
        self.batched_camera_sensor_depth_image = self.batched_camera_sensor.utils.create_depth_image_output(
            self.camera_count, self.sensor_render_width, self.sensor_render_height
        )
        self.batched_camera_sensor_normal_image = self.batched_camera_sensor.utils.create_normal_image_output(
            self.camera_count, self.sensor_render_width, self.sensor_render_height
        )
        self.batched_camera_sensor_shape_index_image = self.batched_camera_sensor.utils.create_shape_index_image_output(
            self.camera_count, self.sensor_render_width, self.sensor_render_height
        )
        ...
        self.semantic_palette = wp.array(
            ...
            dtype=wp.uint8,
            device=self.batched_camera_sensor_color_image.device,
        )
    ...
    def render_sensors(self):
        ...
        self.batched_camera_sensor.update(
            self.state,
            self.get_camera_transforms(),
            self.camera_rays,
            self.camera_indices,
            color_image=self.batched_camera_sensor_color_image,
            albedo_image=self.batched_camera_sensor_albedo_image,
            depth_image=self.batched_camera_sensor_depth_image,
            normal_image=self.batched_camera_sensor_normal_image,
            shape_index_image=self.batched_camera_sensor_shape_index_image,
            clear_data=SensorBatchedCamera.GRAY_CLEAR_DATA,
        )
        utils = self.batched_camera_sensor.utils
        color_rgba = utils.to_rgba_from_color(self.batched_camera_sensor_color_image)
        albedo_rgba = utils.to_rgba_from_color(self.batched_camera_sensor_albedo_image)
        utils.to_rgba_from_depth(
            self.batched_camera_sensor_depth_image, depth_range=(0.0, 10.0), out_buffer=self.depth_rgba
        )
        utils.to_rgba_from_normal(self.batched_camera_sensor_normal_image, out_buffer=self.normal_rgba)
        utils.to_rgba_from_shape_index(self.batched_camera_sensor_shape_index_image, out_buffer=self.shape_rgba)
        utils.to_rgba_from_shape_index(
            self.batched_camera_sensor_shape_index_image, colors=self.semantic_palette, out_buffer=self.semantic_rgba
        )
    ...
    def test_final(self):
        ...
        expected_shape = (self.camera_count, self.sensor_render_height, self.sensor_render_width)

        color_image = self.batched_camera_sensor_color_image.numpy()
        assert color_image.shape == expected_shape
        assert color_image.min() < color_image.max()

        depth_image = self.batched_camera_sensor_depth_image.numpy()
        ...
        albedo_image = self.batched_camera_sensor_albedo_image.numpy()
        assert albedo_image.shape == expected_shape
        assert albedo_image.dtype == np.uint32

        normal_image = self.batched_camera_sensor_normal_image.numpy()
        assert normal_image.shape == (*expected_shape, 3)
        assert normal_image.dtype == np.float32

        shape_index_image = self.batched_camera_sensor_shape_index_image.numpy()
        ...
        albedo_rgba = albedo_image.view(np.uint8).reshape(
            self.camera_count, self.sensor_render_height, self.sensor_render_width, 4
        )
    ...
    def gui(self, ui):
        ...
        if ui.radio_button(
            "Gaussians: Fast",
            self.batched_camera_sensor.render_config.gaussians_mode == SensorBatchedCamera.GaussianRenderMode.FAST,
        ):
            if self.batched_camera_sensor.render_config.gaussians_mode != SensorBatchedCamera.GaussianRenderMode.FAST:
                self.batched_camera_sensor.render_config.gaussians_mode = SensorBatchedCamera.GaussianRenderMode.FAST
                show_compile_kernel_info = True

        if ui.radio_button(
            "Gaussians: Quality",
            self.batched_camera_sensor.render_config.gaussians_mode == SensorBatchedCamera.GaussianRenderMode.QUALITY,
        ):
            if (
                self.batched_camera_sensor.render_config.gaussians_mode
                != SensorBatchedCamera.GaussianRenderMode.QUALITY
            ):
                self.batched_camera_sensor.render_config.gaussians_mode = SensorBatchedCamera.GaussianRenderMode.QUALITY
                show_compile_kernel_info = True
        ...
        changed, value = ui.slider_float(
            "Min Transmittance",
            self.batched_camera_sensor.render_config.gaussians_min_transmittance,
            0.0,
            1.0,
            "%.2f",
        )
        if changed:
            self.batched_camera_sensor.render_config.gaussians_min_transmittance = value
            show_compile_kernel_info = True

        changed, value = ui.slider_int(
            "Max Num Hits",
            self.batched_camera_sensor.render_config.gaussians_max_num_hits,
            1,
            40,
            "%d",
        )
        if changed:
            self.batched_camera_sensor.render_config.gaussians_max_num_hits = value
            show_compile_kernel_info = True
