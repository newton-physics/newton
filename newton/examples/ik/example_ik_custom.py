...
def _collision_residuals(
    ...
    obstacle_centers: wp.array1d[wp.vec3],  # (n_problems,)
    obstacle_radii: wp.array1d[wp.float32],  # (n_problems,)
    link_index: int,
    link_offset: wp.vec3,
    link_radius: float,
    start_idx: int,  # start row in the global residual vector
    weight: float,
    problem_idx: wp.array1d[wp.int32],  # (batch_rows,)
    # outputs
...
def _update_center_target(
    ...
    new_center: wp.vec3,
    # outputs
    centers: wp.array1d[wp.vec3],  # (n_problems,)
):
