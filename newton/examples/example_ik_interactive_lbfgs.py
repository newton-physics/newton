...
def _collision_residuals(
    ...
    tf = body_q[p, link_index]
    ee_pos = wp.transform_point(tf, link_offset)

    # Obstacle sphere
    c = obstacle_centers[p]
    r_obs = obstacle_radii[p]

    dist = wp.length(ee_pos - c)
    delta = (link_radius + r_obs) - dist  # penetration depth
    margin = 0.05
    pen = wp.log(1.0 + wp.exp(delta / margin)) * margin  # softplus(delta, β=1/margin)

    residuals[p, start_idx] = weight * pen
...
class CollisionSphereAvoidObjective(ik.IKObjective):
    ...
    ----------
    link_index : int
        Index of the body whose frame carries the EE sphere.
    link_offset : wp.vec3
        Local-frame offset of the EE sphere centre.
    link_radius : float
        Radius of the EE sphere.
    obstacle_centers : wp.array(dtype=wp.vec3)
        Per-problem world-space centres of the obstacle spheres.
    obstacle_radii : wp.array(dtype=wp.float32)
        Per-problem radii of the obstacle spheres.
    n_problems : int
        Number of parallel IK problems in the batch.
    total_residuals : int
        Length of the global residual vector (only needed for autodiff bookkeeping).
    residual_offset : int
        Start index of this objective's residual inside the global vector.
    weight : float, default 1.0
        Multiplicative weight on the residual.
    """
...
class Example:
    ...
    def __init__(
        ...
        self.joint_q = wp.array(
            self.model.joint_q, shape=(num_envs, self.singleton_model.joint_coord_count), copy=True, requires_grad=True
        )
        ...
        joint_limit_objective = ik.JointLimitObjective(
            ...
            residual_offset=len(self.END_EFFECTOR_NAMES) * 3 * 2 + len(self.END_EFFECTOR_NAMES),
            weight=1.0,
        )

        all_objectives = (
            self.position_objectives + self.rotation_objectives + self.collision_objectives + [joint_limit_objective]
        )

        # IK solver ----------------------------------------------------
        self.ik_solver = ik.LBFGSSolver(
            model=self.singleton_model,
            joint_q=self.joint_q,
            objectives=all_objectives,
            h0_scale=10.0,
            line_search_alphas=[0.1, 0.5, 0.75, 1.0],
            history_len=12,
    ...
    def _build_model(num_envs: int, floating: bool):
        ...
        builder.add_shape_sphere(body=-1, xform=wp.transform(wp.vec3(0.32, 0.8, 0.1), wp.quat_identity()), radius=0.08)
    ...
    def _create_objectives(self):
        num_ees = len(self.END_EFFECTOR_NAMES)
        total_residuals = num_ees * 3 * 2  # pos + rot
        total_residuals += num_ees  # collision
        total_residuals += self.num_dofs  # joint-limit
        ...
        # ------------------------------------------------------------
        # Collision objectives  (one per EE)
        # ------------------------------------------------------------
        collision_objectives = []

        sphere_center_world = wp.vec3(0.32, 0.8, 0.1)
        sphere_radius_world = 0.08

        if self.floating:
            centers = [sphere_center_world] * self.num_envs
        else:
            centers = [sphere_center_world - wp.vec3(*ofs) for ofs in self.env_offsets]
        centers_wp = wp.array(centers, dtype=wp.vec3)
        radii_wp = wp.array([sphere_radius_world] * self.num_envs, dtype=wp.float32)

        collision_row_offset = num_ees * 3 * 2

        for ee_idx, (link_idx, offset) in enumerate(zip(self.ee_link_indices, self.ee_link_offsets)):
            obj = CollisionSphereAvoidObjective(
                link_index=link_idx,
                link_offset=offset,
                link_radius=0.15,
                obstacle_centers=centers_wp,
                obstacle_radii=radii_wp,
                n_problems=self.num_envs,
                total_residuals=total_residuals,
                residual_offset=collision_row_offset + ee_idx,
                weight=10.0,
            )
            collision_objectives.append(obj)

        return position_objectives, rotation_objectives, collision_objectives, total_residuals
