...
def _as_vec2_array(xy_m: np.ndarray, device: str | wp.context.Device | None, requires_grad: bool) -> wp.array:
    ...
    return wp.array([wp.vec2(float(x), float(y)) for x, y in xy], dtype=wp.vec2, device=device, requires_grad=requires_grad)
...
def fit_foundation_material_autodiff(
    ...
    *,
    ...
    for iteration in range(iterations):
        ...
        relative_step = rates * safe_grad * np.maximum(np.abs(params), 1.0) / max(mean_loss, 1.0)
        relative_step = np.clip(relative_step, -0.25, 0.25)
        params = params - relative_step * np.maximum(np.abs(params), 1.0)
        params = _material_to_array(_array_to_material(params))

    return FoundationFitResult(material=_array_to_material(params), history=tuple(history))
