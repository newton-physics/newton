...
def test_edge_bending_damping():
    ...
    for i in range(grid_size - 1):
        for j in range(grid_size - 1):
            ...
            builder.add_triangle(
                get_idx(i, j), get_idx(i+1, j), get_idx(i, j+1),
                tri_ke=1000.0, tri_kd=0.0, tri_ka=1000.0
            )
            builder.add_triangle(
                get_idx(i+1, j), get_idx(i+1, j+1), get_idx(i, j+1),
                tri_ke=1000.0, tri_kd=0.0, tri_ka=1000.0
            )
    ...
    for i in range(grid_size - 2):
        for j in range(grid_size - 1):
            # Horizontal edges
            builder.add_edge(
                get_idx(i, j), get_idx(i+1, j), 
                get_idx(i, j+1), get_idx(i+1, j+1),
                rest=0.0,  # rest angle
