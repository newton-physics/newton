...
- Derive each cable joint's stiffness in the experimental USD deformable importer from its own adjacent segment rest lengths (`0.5 * (L_parent + L_child)`) instead of one curve-wide mean segment length. Cables whose segments are not uniformly sampled import with different per-joint stiffness than in 1.4.0; re-test or re-tune those scenes. (#3722)
