...
- Add `ModelBuilder.deformable_label` and `ModelBuilder.deformable_offset` registry tracking each cloth/soft body added via `add_cloth_mesh`, `add_cloth_grid`, `add_soft_mesh`, or `add_soft_grid` (and the Style3D variants); each function gains an optional `label` parameter, defaulting to `"<source>_<deformable_count>"` (e.g. `"cloth_mesh_0"`), and the label flows into the `validate_*_mesh` warning text for diagnostics
