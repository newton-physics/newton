...
- Require keyword arguments for solver constructor option parameters after stable positional inputs such as `model` and explicit solver configs; migrate calls such as `SolverVBD(model, 10)` to `SolverVBD(model, iterations=10)`.
- Require keyword arguments for additional configuration and filter options: use `ShapeConfig(mu=0.5)`, `JointDofConfig(axis=Axis.X)`, `Contacts(rigid_contact_max, soft_contact_max, device=device)`, and `ArticulationView(model, pattern, include_joints=[...])`; `ModelBuilder.add_body()` and `add_link()` keep `xform` positional but require later options such as `mass=` by keyword, and `add_shape_gaussian()` requires options after `body` by keyword.
