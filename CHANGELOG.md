...
- Fix `SolverMuJoCo` ball-joint conversion so that `joint_q`, `joint_qd`, and applied/actuator `joint_f` on a joint with non-identity `child_xform` rotation produce matching body transforms, angular velocities, and angular accelerations across `newton.eval_fk`, `mujoco.mj_kinematics`, and `mujoco_warp.kinematics`. The previous xyzw-to-wxyz layout swap did not bridge Newton's anchor-frame quaternion to MuJoCo's body-frame `qpos`/`qvel`/`qfrc`; the bridge kernels now conjugate by `joint_X_c.q`.
