...
- Deprecate `newton.EqType` in favor of `newton.solvers.SolverMuJoCo.EqType`; migrate equality-constraint type references to the MuJoCo-scoped enum.
 Deprecate `newton.geometry.MATCH_BROKEN` and `newton.geometry.MATCH_NOT_FOUND`; these were implementation-detail sentinels with no public replacement. Do not compare `rigid_contact_match_index` against them directly, and avoid importing them going forward.
