...
- Add `newton.selection.DeformableView` for batched, label-pattern access to imported deformables: select cable, cloth, or soft-body groups by prim path across replicated worlds and read or write their particle positions/velocities and cable body transforms/velocities as `(count, elements_per_group)` arrays. The finalized `Model` now also carries the per-family group metadata (`cable_*` / `cloth_*` / `soft_*` labels, worlds, and index ranges) with `Model.cable_index()` / `cloth_index()` / `soft_index()` lookups that take an explicit `world` when replication duplicates a label.
