...
- Add the experimental `newton.selection.DeformableView` for label-pattern access to finalized curve, surface, and volume deformables. Infer unambiguous families, read through zero-copy or internally staged selection layouts, update flat group subsets, recover uneven per-world partitions, and inspect raw ranges for ragged groups. Native builder calls and USD imports use the same selection API; the `selection_deformables` example demonstrates labels, range access, and indexed resets.
