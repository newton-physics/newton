...
- Change `Model.shape_collision_filter_pairs` to a read-only set; modify `ModelBuilder.shape_collision_filter_pairs` before calling `finalize()` and rebuild the model instead of mutating finalized collision filters in place.
