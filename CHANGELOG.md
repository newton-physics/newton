...
- Treat `stretch_stiffness` and `bend_stiffness` in `add_rod()` and `add_rod_graph()` as direct per-joint stiffness values, matching `add_joint_cable()` and other joint stiffness APIs. Migration: previously these values were divided by segment length internally; divide your old explicit values by `segment_length` to preserve the same effective per-joint stiffness.
- Change the cable rod body-frame convention generated by `ModelBuilder.add_rod()` and `add_rod_graph()` so each capsule body origin coincides with its capsule COM. Migration: update custom rod endpoint anchors to use midpoint-relative coordinates.
...
- Improve mouse picking in cable examples by applying viewer drag as translation-only forces on cable capsule bodies, avoiding torque-induced instability while preserving linear dragging.
- VBD solver uses augmented-Lagrangian hard constraints for body-body contacts and non-cable structural joint slots by default; cable joints remain stiffness-driven. Pass `rigid_contact_hard=False` to `SolverVBD(...)` to restore the previous soft-contact behavior.
