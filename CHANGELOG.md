...
- Import authored MPM particle sets from `UsdGeom.Points` carrying `NewtonMPMParticleAPI`, including SI unit and world-transform conversion, density- and width-derived masses, whole-prim and point-`GeomSubset` physics material bindings, per-prim particle ranges, and `SolverImplicitMPM.Config.create_from_usd()` scene configuration. Require every resolved MPM physics material to carry `NewtonMPMMaterialAPI`; unbound particle sets use Newton's registered material defaults and the builder's default density.
