...
- Fix `SolverMuJoCo` CPU backend preserving `mj_model.body_iquat` while overwriting `mj_model.body_inertia` with a differently ordered eigendecomposition; the CPU model now keeps the compiled inertia frame when it still represents Newton's inertia tensor, preserving non-descending MJCF `diaginertia` axis associations
