...
- Add `newton.utils.is_graph_capture_allocation_enabled(device)` reporting whether device allocation during graph capture is currently safe: always `True` on CPU (host allocation is unaffected by capture), and on CUDA gated on the mempool being enabled.
