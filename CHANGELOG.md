...
- Add full-surface (edge/face) rigid-soft contact support to `SolverVBD` proxy-body coupling under `SolverCoupledProxy`. Enable it on the proxy's own collision pipeline (`SolverCoupledProxy.Proxy(collision_pipeline=...)`). Proxy particles do not support full-surface contacts and raise.
...
- Reject full-surface (edge/face) soft contacts on the contact buffer passed to `SolverCoupled`, which previously accepted them and silently dropped the edge/face records. Build that pipeline with `enable_rigid_soft_full_surface_contact=False` and enable the flag on the proxy's `collision_pipeline` instead.
