...
- Import USD deformable bodies in `ModelBuilder.add_usd()` (experimental; pre-release coverage of the proposed, non-registered [AOUSD Deformable Body Physics schema](https://github.com/aousd/OpenUSD-proposals/blob/5d89c0ed46a26de92f4d3fefef3bfad6500c07ce/proposals/physics_deformables/wp_deformable_physics.md)) across all three families: curve/cable (`GeomBasisCurves` + `PhysicsCurvesDeformableSimAPI` built as a VBD rod of capsule bodies and cable joints, using per-vertex normals for segment orientation; each cable is wrapped into its own articulation so the model is finalize-ready), surface/cloth (`GeomMesh` + `PhysicsSurfaceDeformableSimAPI` built as FEM triangles with bending edges, fan-triangulating polygonal faces such as quads), and volume (`UsdGeom.TetMesh` + `PhysicsVolumeDeformableSimAPI` built as a soft body). Bound deformable materials supply per-family thickness, stiffness, and density, and the proposal's mass-distribution precedence is honored (per-point `physics:masses`, then `PhysicsDeformableBodyAPI` `mass`/`density`, then material density). Each imported deformable is a prim-path-labelled, world-tagged group on `ModelBuilder` and the finalized `Model` with `[start, end)` index ranges (cable body/joint, cloth particle/triangle/edge, volume particle/tet) that survive `finalize()`, `replicate()`, and `collapse_fixed_joints`; look one up with `Model.cable_index()` / `cloth_index()` / `soft_index()` (with an explicit `world` when replication duplicates a label) and the matching `*_range()` helpers. Disabled (`physics:bodyEnabled = false`) and kinematic (`physics:kinematicEnabled = true`) deformables warn and are skipped rather than imported as dynamic objects. `add_usd()` also returns `path_cable_attrs`, `path_cloth_attrs`, and `path_soft_attrs` exposing each deformable's as-authored, solver-neutral attributes (cable/cloth: `material` moduli and `resolved_density`; volume: `resolved_density`), including moduli the isotropic `SolverVBD` / `SolverSemiImplicit` membranes cannot apply (cable shear/twist; cloth `shearStiffness`), so an anisotropic solver such as `SolverStyle3D` can rebuild from the import.
- Import supported AOUSD proposal `PhysicsAttachment` prims for cable-to-xform attachments in `ModelBuilder.add_usd()`: cable `point` / `segment` sites with `type1 = "xform"` become hard point ball joints to the target xform, rigid body (including a kinematic body), or world frame and are returned in `path_attachment_map`; unsupported cloth/volume attachment sites are warned and preserved in `path_attachment_attrs`. A hard (unauthored/infinite stiffness), coincident `point`->`point` attachment between two imported curve deformables is instead treated as graph topology: transitively joined curves are welded at the shared control points and built as one rod via `ModelBuilder.add_rod_graph()` (pre-wrapped in a single articulation); a compliant or non-coincident curve junction warns and is preserved as unsupported instead of welded.
- Import AOUSD proposal `PhysicsElementCollisionFilter` prims in `ModelBuilder.add_usd()`: suppress collisions between paired element groups of `src0` and `src1`, lowered to shape collision-filter pairs. `groupElemCounts0` / `groupElemCounts1` slice the flat `groupElemIndices` arrays into groups that pair element-wise (collisions filtered only within each paired group, not the full cross product); a count of `0`, or an empty counts array, means all elements of that source. Sources resolve to imported cables (indices select the cable's segments), rigid bodies (all collider shapes), or collider prims (a child collider under a rigid Xform, or a bodyless static collider); cloth/volume (triangle/tet) element sources are warned and skipped.
...
- Deprecate the default of `newton.usd.get_tetmesh()` and `newton.TetMesh.create_from_usd()` reading legacy vendor-namespaced deformable material attributes (`omniphysics:`, `physxDeformableBody:`) off any bound material. The default still reads them during the deprecation window but now emits a `DeprecationWarning` when a physics material is bound; the default will change to canonical `physics:`-only in a future release. Pass `compat_namespaces=()` to opt into the canonical-only behavior now (moduli read only from a material applying `PhysicsVolumeDeformableMaterialAPI`), or `compat_namespaces=newton.usd.DEFORMABLE_LEGACY_NAMESPACES` to keep reading vendor namespaces without the warning. `compat_namespaces` is now keyword-only.
