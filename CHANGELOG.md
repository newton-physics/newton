...
- Warn from `SolverMuJoCo` when non-default finite `Model.joint_velocity_limit` values are ignored because MuJoCo has no equivalent joint velocity clamp. Use `joint_effort_limit`, joint damping/controller limits, or a custom actuator to bound speed.
