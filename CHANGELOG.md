...
- Change the cable rod body-frame convention generated by `ModelBuilder.add_rod()` and `add_rod_graph()` so each capsule body origin coincides with its capsule COM; update custom rod endpoint anchors to use midpoint-relative coordinates.
...
- VBD solver uses augmented-Lagrangian hard constraints for body-body contacts and non-cable structural joint slots by default; cable joints remain stiffness-driven
