...
- Add the experimental `newton.selection.DeformableView` for label-pattern access to finalized curve, surface, and volume deformables. Read or write particle and curve-body state on a flat group axis, recover uneven per-world partitions through `world_starts`, update explicit groups or one matching group per world, and use raw `[start, end)` ranges for deformables with different element counts. Labeled native builder calls, including soft grids, and USD imports use the same selection API; the `selection_deformables` example demonstrates procedural labeling, range access, and indexed resets.
