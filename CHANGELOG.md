...
- Fix `ModelBuilder.add_usd()` placing world-target `PhysicsAttachment` anchors in original USD coordinates so they ignored the import `xform`, pulling imported cables off-position; the world anchor now rides the same import transform as the cable geometry.
