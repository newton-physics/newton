...
- Fix mesh collision sign queries to use parity for watertight meshes and the normal-based method for non-watertight meshes; set `ShapeFlags.MESH_SIGN_NORMAL` or `ShapeFlags.MESH_SIGN_PARITY` to force the respective method for both runtime queries and SDFs baked at model finalization (`ShapeFlags.MESH_SIGN_AUTO`, the default, selects from mesh topology). (#3242)
