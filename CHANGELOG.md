...
- Warn from `ModelBuilder.add_joint` when a `JointType.FREE` joint is given a non-world parent, surfacing models that `SolverMuJoCo` cannot compile (free joints must attach directly to the world)
