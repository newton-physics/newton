...
- Change experimental `SolverVBD` to rebuild rigid previous-pose baselines each step. Non-kinematic pose edits now reposition bodies without producing velocity or contact friction. For friction-driving prescribed motion, set `is_kinematic=True` and provide `State.body_qd`; `mass=0` alone no longer suffices.
