...
- USD importer warns and disables hydroelastic on a Mesh prim with `newton:hydroelasticEnabled=true` but no SDF source (no `sdfMaxResolution` / `sdfTargetVoxelSize`), instead of aborting the entire import. The shape falls back to a plain mesh collider.
- USD importer warns and drops invalid SDF configuration inline instead of aborting the whole import. Catches `newton:sdfMaxResolution` not divisible by 8, unknown `newton:sdfTextureFormat`, and both `newton:sdfMaxResolution` + `newton:sdfTargetVoxelSize` authored on the same prim (target voxel size takes precedence in the last case). The Python API path (`ShapeConfig.validate()`) still raises.
