...
- Add `JointType.ROD` and `ModelBuilder.add_joint_rod()` as the canonical rod-joint names, plus consistently prefixed `newton.utils.cable_*` cable-construction helpers; the previous names remain available with deprecation warnings in 1.6.
...
- Add optional `shear_stiffness`/`shear_damping` and `twist_stiffness`/`twist_damping` controls to `ModelBuilder.add_joint_rod()`, `ModelBuilder.add_rod()`, and `ModelBuilder.add_rod_graph()`; omitted shear defaults to stretch and omitted twist defaults to bend for compatibility.
- Add `newton.utils.CableStiffness` and extend `newton.utils.cable_stiffness_from_elastic_moduli()` with `poissons_ratio`/`shear_modulus` inputs that include torsional `GJ/L` stiffness.
...
- Make `ROD` the canonical `JointType` name for integer value 7. Code serializing `JointType(7).name` now emits `"ROD"`; accept the deprecated `"CABLE"` name when reading older name-based data.
...
- Deprecate `JointType.CABLE` and `ModelBuilder.add_joint_cable()` in 1.6 in favor of `JointType.ROD` and `ModelBuilder.add_joint_rod()`; the old names remain functional through 1.6 and are eligible for removal in 1.7.
...
- Deprecate the `newton.utils.create_*` cable helpers in 1.6 in favor of `cable_stiffness_from_elastic_moduli()`, `cable_parallel_transport_quaternions()`, `cable_straight_points()`, and `cable_straight_points_and_quaternions()`; the old names remain functional through 1.6 and are eligible for removal in 1.7.
