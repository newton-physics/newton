...
- Add an optional `label` parameter to `ModelBuilder.add_cloth_mesh`, `add_cloth_grid`, `add_soft_mesh`, `add_soft_grid`, and the Style3D variants; the label is forwarded to `validate_triangle_mesh` / `validate_tet_mesh` and prefixes the warning text in brackets so multiple cloth/soft meshes in one builder can be told apart in diagnostics
