...
- Add `ModelBuilder.MeshApproximationConfig` and `ModelBuilder.default_mesh_approximation_cfg` to configure automatic mesh approximation during USD import; set `builder.default_mesh_approximation_cfg.coacd_threshold = 0.5` before `add_usd()` to restore the previous coarse CoACD decomposition. (#3444)
...
- Change the default CoACD convex decomposition threshold from `0.5` to `0.05` to match CoACD's default; pass `threshold=0.5` to `ModelBuilder.approximate_meshes()`, or set `builder.default_mesh_approximation_cfg.coacd_threshold = 0.5` before `add_usd()`, to preserve the previous coarse decomposition.
