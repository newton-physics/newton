...
- Warn from `SolverMuJoCo` when a `JointType.FREE` joint has a non-world parent; MuJoCo requires free joints to attach directly to the world, and the warning is emitted before the compile error so users get a clear diagnostic
