...
- Add `tendon_mujoco_wrap` example demonstrating fixed-topology dynamic tendon wrap activation
- Add `tendon_mujoco_switch` example demonstrating active-set switching between a lower guide-only route and an optional middle capstan route
- Add `tendon_mujoco_switch_matrix` example covering attachment/rolling neighbor combinations for active-set route switching
- Add tendon active-set routing so initially inactive rolling links can be skipped and later reactivated by the solver route state
...
- Fix XPBD tendon capstan friction with per-iteration rolling-contact coupling and kinematic rest-transfer projection against the Euler-Eytelwein tension bound
- Fix XPBD tendon relaxation so accumulated cable multipliers and reported tensions are not scaled by `joint_linear_relaxation`
