...
- Add `newton.usd.runtime` with `load_usd()` and `Simulation.step()` for stage-driven simulation: solver selection and configuration are authored on the USD `PhysicsScene` prim via applied `NewtonSolver*API` schemas
...
- Fix spurious custom-attribute warnings when importing USD stages that author plain `newton:` attributes without customData
