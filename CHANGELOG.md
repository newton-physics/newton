...
- Import authored AOUSD cable stretch, shear, bend, and twist stiffness independently through `ModelBuilder.add_usd()`.
...
- Derive each cable joint's stiffness in the experimental USD deformable importer from its own adjacent rest lengths (`0.5 * (L_parent + L_child)`) instead of one curve-wide mean, and honor a welded graph's authored `restShapePoints` instead of dropping it. Unevenly sampled cables, and welded cables with an authored rest shape, import with different stiffness than in 1.4.0; re-test those scenes. An unusable `restShapePoints` now warns instead of being discarded silently. (#3722)
- Read AOUSD's current cable material attributes in the experimental USD deformable importer: `physics:curvesThickness` and the four `physics:curves*Stiffness` structural stiffnesses, deriving any unauthored mode from `physics:youngsModulus` and `physics:poissonsRatio` instead of falling back to `add_rod` defaults. A curve material with no authored thickness now assumes a 0.5 mm radius rather than Newton's 2.5 mm; re-test affected scenes, and see Deprecated for migrating the earlier unprefixed attributes.
