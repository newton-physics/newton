...
- Add forward/inverse kinematics for `JointType.CABLE`: cable `joint_q` now stores the full relative anchor pose and `joint_qd` stores its six-dimensional relative twist, so `newton.eval_fk` reconstructs cable bodies and velocities and `newton.eval_ik` recovers their joint state like other joints (previously `eval_fk` left cable bodies untouched). The current isotropic linear and angular VBD material coefficients are replicated over the corresponding three-axis DOF blocks; `SolverVBD` still owns cable dynamics.
