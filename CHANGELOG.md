...
- MJCF and USD importers no longer subtract `gap` from `margin` to compute `Model.shape_margin`. Under MuJoCo 3.9.0, MJCF `margin`/`gap` and USD `mjc:margin`/`mjc:gap` semantics match Newton's `shape_margin` and `shape_gap`, so import is an identity translation. Pass `legacy_margin_gap=True` to `ModelBuilder.add_mjcf` or `ModelBuilder.add_usd` to restore the previous behavior for files authored against MuJoCo <= 3.8.
...
- Upgrade `mujoco` and `mujoco-warp` dependencies to 3.9.0. `Model.shape_gap` is now propagated into MuJoCo's `geom_gap` and `pair_gap`, so contact-detection ranges authored on Newton shapes are honored by MuJoCo's broadphase. Previously these were zeroed; users who want MuJoCo's broadphase to ignore `shape_gap` should explicitly set it to zero.
