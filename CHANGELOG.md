...
- Add runtime per-shape texture swapping to the viewer: `ViewerGL.register_textures(sources)` uploads candidate textures once into a shared texture array, and `ViewerBase.update_shape_textures(shape_indices, texture)` reassigns them per shape instance at runtime (e.g. per-reset domain randomization) without decoding or re-uploading pixels; shapes sharing deduplicated geometry can render different textures. Unregistered sources fall back to a per-logged-mesh upload via `update_mesh_texture`.
