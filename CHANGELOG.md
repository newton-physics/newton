...
- Change cable capsules generated by `ModelBuilder.add_rod()` and `add_rod_graph()` so each segment body origin coincides with its capsule COM, replacing the previous segment-start origin convention.
