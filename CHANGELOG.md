...
- Change USD import resolution of `newton:limitStiffness` and `newton:limitDamping` sentinels: an authored `-inf` now selects the `ModelBuilder` engine default with precedence over lower-priority per-DOF gains from PhysX or MuJoCo resolvers. Previously an authored `-inf` was indistinguishable from "not authored" and could be overridden by a lower-priority schema.
