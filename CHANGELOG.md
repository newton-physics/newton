...
- Import USD deformable bodies in `ModelBuilder.add_usd()` across all three families: curve/cable (`GeomBasisCurves` + `PhysicsCurvesDeformableSimAPI` built as a VBD rod of capsule bodies and cable joints, using per-vertex normals for segment orientation), surface/cloth (triangulated `GeomMesh` + `PhysicsSurfaceDeformableSimAPI` built as FEM triangles with bending edges), and volume (`UsdGeom.TetMesh` + `PhysicsVolumeDeformableSimAPI` built as a soft body). Bound deformable materials supply per-family thickness, stiffness, and density, and the proposal's mass-distribution precedence is honored (per-point `physics:masses`, then `PhysicsDeformableBodyAPI` `mass`/`density`, then material density). `add_usd()` returns `path_cable_map`, `path_cloth_map`, and `path_soft_map` mapping each prim path to its body/joint or particle/element ranges for per-instance addressability.
...
- Add an optional `newton:cableWrapInArticulation` (bool, default `true`) Newton-extension attribute read by `ModelBuilder.add_usd()` on curve-deformable (cable) prims when the Newton schema resolver is active. When `false`, the imported cable joints are not wrapped in an articulation, letting the caller place them (e.g. close a loop or attach the cable to other bodies) before `finalize()`.
