...
- Add opt-in `validate_mesh` parameter to `ModelBuilder.add_cloth_mesh()`, `ModelBuilder.add_soft_mesh()`, and `style3d.add_cloth_mesh()` that warns on degenerate geometry; add public `newton.utils.validate_triangle_mesh()` and `newton.utils.validate_tet_mesh()` utilities
- Add `cloth_stiff_material_hanging` example: stiff square cloth (`tri_ke = tri_ka = 1e8`) pinned along one edge under gravity, exercising the new Neo-Hookean triangle material at parameters where the previous StVK form would explode. `test_final` checks for non-explosion via a per-particle velocity bound in addition to the existing position-volume checks
- Add `cloth_stiff_material_stretch` example: five cloth sheets pinned at +/-X edges and stretched, each with a different effective Poisson ratio set via `tri_ka / tri_ke`. `test_final` checks bulk area against the closed-form StableNH 2D-membrane prediction within 10%, regression-guarding the new triangle material model
...
- Add `soft_beam_extension`, `soft_beam_stretch`, `soft_beam_twist`, `soft_cube_compression`, `soft_convergence_refinement`, and `soft_sliver_elements` volumetric examples regression-guarding Neo-Hookean tet-mesh elasticity (extension under gravity, volume preservation under stretch and twist, recovery from extreme compression, mesh-convergence sanity, and sliver-element robustness)
