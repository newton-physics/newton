...
- Require keyword arguments for solver constructor parameters after `model`; migrate calls such as `SolverVBD(model, 10)` to `SolverVBD(model, iterations=10)` and `SolverImplicitMPM(model, config)` to `SolverImplicitMPM(model, config=config)`.
