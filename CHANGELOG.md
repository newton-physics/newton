...
- Add `sign_method` argument to `Mesh.build_sdf` for selecting the inside/outside sign strategy of the baked SDF (`"auto"`, `"parity"`, or `"winding"`).
...
- Fix mesh collision sign queries to use parity for watertight meshes and the normal-based method for non-watertight meshes; set `ShapeFlags.MESH_SIGN_NORMAL` or `ShapeFlags.MESH_SIGN_PARITY` to force non-watertight or watertight treatment for both runtime queries and SDFs baked at model finalization (`ShapeFlags.MESH_SIGN_AUTO`, the default, selects from mesh topology). (#3242)
