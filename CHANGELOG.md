...
- Add the experimental `newton.selection.DeformableView` for label-pattern access to finalized curve, surface, and volume deformables. Read or write existing particle and curve-body state in batches, inspect stable generated group identities, and use raw `[start, end)` ranges for groups with different element counts. Labeled native builder calls and USD imports use the same selection API.
