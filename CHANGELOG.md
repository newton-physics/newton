...
- Derive each cable joint's stiffness in the experimental USD deformable importer from its local dual rest length, `0.5 * (L_parent + L_child)`, instead of a curve-wide mean, and use a welded graph's authored `restShapePoints` for that normalization. Re-test unevenly sampled or welded cables with authored rest shapes; their imported stiffness may differ from 1.4.0. An invalid `restShapePoints` value now emits a warning instead of being ignored silently. (#3722)
- Read the current AOUSD cable material attributes in the experimental USD deformable importer: `physics:curvesThickness` and the four structural `physics:curves*Stiffness` attributes. Resolve each unauthored stiffness from `physics:youngsModulus` and `physics:poissonsRatio` instead of using `add_rod()` defaults. A curve material with unauthored thickness now uses AOUSD's 1 mm diameter fallback (0.5 mm radius) instead of Newton's previous 2.5 mm radius; re-test affected scenes and see Deprecated below for migration from the earlier unprefixed attributes.
