...
- Import AOUSD proposal `PhysicsElementCollisionFilter` prims in `ModelBuilder.add_usd()`: suppress collisions between the authored element groups of imported cables (`groupElemIndices` index a cable's segments) and rigid colliders (an empty index array means all of that source), lowered to shape collision-filter pairs; cloth/volume (triangle/tet) element sources are warned and skipped.
