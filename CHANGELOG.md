...
- Change experimental `SolverVBD` to rebuild rigid previous-pose baselines each step. Manual non-kinematic pose edits are now clean teleports and do not contribute pose-derived velocity or frictional motion. For prescribed motion, set `is_kinematic=True` and author consistent `State.body_q` and `State.body_qd` every step or substep; `mass=0` alone does not make a body kinematic.
