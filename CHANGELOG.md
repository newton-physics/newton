...
- Add `ModelBuilder.add_deformable_render_mesh()` to embed a high-resolution textured render mesh in a coarse deformable (cloth via per-particle binding, or volumetric soft bodies via barycentric tet embedding). The render mesh is skinned from the simulation state on-device each frame and drawn through `log_mesh`, so it renders in every viewer backend; toggle with `viewer.show_render_mesh` and color by displacement with `viewer.show_render_strain`. USD import attaches render meshes targeted by a `newton:renderMesh` relationship on a deformable. See `example_softbody_render_mesh.py` and `newton.DeformableRenderMesh`.
- Add `ModelBuilder.add_deformable_render_mesh()` to embed a high-resolution textured render mesh in a coarse deformable (cloth via per-particle binding, volumetric soft bodies via barycentric tet embedding, or cables/rods via rigid-body binding). The render mesh is skinned from the simulation state on-device each frame and drawn through `log_mesh`, so it renders in every viewer backend; toggle with `viewer.show_render_mesh` and color by displacement with `viewer.show_render_strain`. USD import attaches render meshes targeted by a `newton:renderMesh` relationship on a deformable. See `example_softbody_render_mesh.py`, `example_cloth_render_mesh.py`, `example_cable_render_mesh.py`, and `newton.DeformableRenderMesh`.
