...
the simulation-sized array it views. Capture works on CPU or CUDA devices, and
the C++ runtime replays on whichever device the graph was captured for,
detected from the `.wrp` file itself. The C++ side is built separately with
CMake and is not part of `pip install newton`; see the README in that
directory.
