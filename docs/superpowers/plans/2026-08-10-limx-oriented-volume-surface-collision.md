...
**Architecture:** Extend `ConstraintSelfCollision` with a backward-compatible `use_outward_normals` flag. In that mode, filter topology-local pairs, freeze oriented signed contact data during `prepare()`, and reuse the existing balanced force and PSD Hessian assembly.
...
- Filter one-ring VF and topology-local EE only in oriented mode.
...
- [ ] **Step 2: Add a one-ring exclusion regression test**

Construct two adjacent triangles whose opposite vertex projects inside its
neighbor within thickness. Assert that oriented mode stores no matching VF
contact, while the existing default-path tests remain unchanged.
...
flag is enabled, reject one-ring candidates, keep the outward triangle normal,
and store `effective_thickness - signed_distance`. Keep the old absolute-value
...
- [ ] **Step 2: Add a topology-local EE exclusion test**
...
to the EE kernel. In oriented mode, reject topology-local pairs, compute both
edge pseudo-normals from current incident face normals, set direction to
...
- Produces: an oriented, frictionless, VF/EE-only eight-bunny experiment.
