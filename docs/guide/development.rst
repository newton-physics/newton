...
It also keeps the API Reference toctree in ``docs/index.rst`` in sync with the script's
``MODULES`` list, so new top-level modules appear in the docs sidebar without a hand-edit.
