...
Masked resets preserve both this cadence and cached contacts, and mark
selected-world matching history for invalidation on the next refresh. If the
factory returns ``None`` or no factory is supplied, the destination solve uses
contacts passed to the outer ``step()`` call.
