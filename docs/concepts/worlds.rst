...
The world-relative index is useful in certain operations. For example, it is used when accessing the body-specific column of constraint Jacobian matrices in maximal-coordinate formulations. These matrices are stored in contiguous blocks per world. This relative index can then be mapped to the global entity index within the model by adding the corresponding starting index from the ``*_world_start`` arrays.
