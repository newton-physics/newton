...
- **Integrator.** ``SolverMuJoCo`` defaults to ``integrator="implicitfast"``,
  which is a useful starting point for stiff joint drives. Switch to
  ``"euler"`` only for a specific reason, and expect to reduce ``dt`` if needed.
- **Contact path** (``use_mujoco_contacts``, default ``True``). ``True`` uses
  MuJoCo's collision detection; ``False`` accepts contacts generated by Newton's
  collision pipeline. Choose one and tune within it rather than mixing
