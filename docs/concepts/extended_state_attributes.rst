...
These are called extended state attributes and are allocated by the :meth:`Model.state <newton.Model.state>` method, if they have been requested on the model or builder.
...
- Extended state attributes are optional and allocated and computed only if you request them before calling :meth:`Model.state() <newton.Model.state>`.
...
- Solvers only populate optional outputs they explicitly support. When an extended state attribute is allocated on the State, a supporting solver will update it during its step().
  Currently, :class:`~newton.solvers.SolverMuJoCo` supports populating ``body_qdd`` and ``body_parent_f``.
